import {Component} from "../src/viewer/Component";
import * as math from "../src/viewer/math";
import {Mesh} from './lib/Mesh';
import {FrameContext} from "../src/WebGLRenderer/FrameContext";
import {WebGLRenderer} from "../src/WebGLRenderer/WebGLRenderer";
import {Scene} from "../src/viewer/scene/Scene";
import {View} from "../src/viewer/view/View";
import {Geometry} from "./lib/Geometry";
import {SceneModel} from "../src/viewer/scene/SceneModel";
import {DrawFlags} from "../src/WebGLRenderer/DrawFlags";
import {TextureSet} from "./lib/TextureSet";
import {getScratchMemory, putScratchMemory, ScratchMemory} from "./lib/ScratchMemory";
import {Texture} from "./lib/Texture";
import {Texture2D} from "../src/WebGLRenderer/lib/Texture2D";
import {Events} from "../src/viewer/Events";
import {Transform} from "./lib/Transform";
import {createUUID, loadArraybuffer} from "../src/viewer/utils";
...
import {Drawable} from "../src/WebGLRenderer/Drawable";
import {collapseAABB3, expandAABB3} from "../src/viewer/math/boundaries";
import {getKTX2TextureTranscoder} from "../src/WebGLRenderer/textureTranscoders/KTX2TextureTranscoder/KTX2TextureTranscoder";
...
import {
    GeometryParams, GeometryCompressedParams,
    MeshParams,
    Renderer,
    SceneObjectParams,
    TextureParams,
...
export class WebGLSceneModel extends Component implements SceneModel, Drawable {
    ...
    createGeometry(params: GeometryParams ): void {
        ...
        if (primitive !== PointsPrimitive && primitive !== LinesPrimitive && primitive !== TrianglesPrimitive && primitive !== SolidPrimitive && primitive !== SurfacePrimitive) {
            this.error(`[createGeometry] Unsupported value for 'primitive': '${primitive}'`);
            return;
        }
        ...
        if (params.positionsCompressed && !params.positionsDecompressMatrix) {
            this.error("[createGeometry] Param expected: `positionsDecompressMatrix` (required for `positionsCompressed')");
            return null;
        }
        if (params.uvsCompressed && !params.uvsDecompressMatrix) {
            this.error("[createGeometry] Param expected: `uvsDecompressMatrix` (required for `uvsCompressed')");
            return null;
        }
        if (!params.indices && primitive !== PointsPrimitive) {
            this.error(`Param expected: indices (required for primitive type)`);
            return null;
        }
    ...
    createGeometryCompressed(geometryCompressedParams: GeometryCompressedParams ): void {
        ...
        if (geometryId === undefined || geometryId === null) {
            this.error("[createGeometryCompressed] Config missing: id");
            return;
        }
        if (this.#geometries[geometryId]) {
        ...
        if (primitive !== PointsPrimitive && primitive !== LinesPrimitive && primitive !== TrianglesPrimitive && primitive !== SolidPrimitive && primitive !== SurfacePrimitive) {
            this.error(`[createGeometryCompressed] Unsupported value for 'primitive': '${primitive}'`);
            return;
        }
        if (!geometryCompressedParams.positionsCompressed) {
            this.error("[createGeometryCompressed] Param expected: `positionsCompressed'");
            return null;
        }
        if (!geometryCompressedParams.positionsDecompressMatrix) {
            this.error("[createGeometryCompressed] Param expected: `positionsDecompressMatrix`");
            return null;
        }
        if (geometryCompressedParams.uvsCompressed && !geometryCompressedParams.uvsDecompressMatrix) {
            this.error("[createGeometryCompressed] Param expected: `uvsDecompressMatrix` (required for `uvsCompressed')");
            return null;
        }
        if (!geometryCompressedParams.geometryBuckets) {
            this.error(`[createGeometryCompressed] Param expected: geometryBuckets`);
            return null;
        }
        const geometry = new Geometry(this.#webglRenderer.gl, geometryCompressedParams);
        this.#geometries[geometryId] = geometry;
        if (primitive === TrianglesPrimitive || primitive === SolidPrimitive || primitive !== SurfacePrimitive) {
            this.#numTriangles += (geometryCompressedParams.indices ? Math.round(geometryCompressedParams.indices.length / 3) : 0);
        }
        this.#numGeometries++;
    }
