import {Camera, math, SceneModel} from "../../../../src/viewer";
...
export class DataTextureFactory {
    ...
    /**
     * Create a DataTexture containg the given Camera's viewMatrix, viewNormalMatrix and projectMatrix
     */
    createCameraMatricesDataTexture(gl: WebGL2RenderingContext, camera: Camera, origin: math.FloatArrayType): DataTexture {
     ...
     */
    createPositionsDataTexture(gl: WebGL2RenderingContext, positions: math.FloatArrayType): DataTexture {
     ...
     */
    createIndices8BitDataTexture(gl: WebGL2RenderingContext, indices_8Bits: math.IntArrayType): DataTexture {
     ...
     */
    createIndices16BitDataTexture(gl: WebGL2RenderingContext, indices_16Bits: math.IntArrayType): DataTexture {
     ...
     */
    createIndices32BitDataTexture(gl: WebGL2RenderingContext, indices_32Bits: math.IntArrayType): DataTexture {
     ...
     */
    createEdgeIndices8BitDataTexture(gl: WebGL2RenderingContext, edgeIndices_8Bits: math.IntArrayType): DataTexture {
     ...
     */
    createEdgeIndices16BitDataTexture(gl: WebGL2RenderingContext, edgeIndices_16Bits: math.IntArrayType): DataTexture {
     ...
     */
    createEdgeIndices32BitDataTexture(gl: WebGL2RenderingContext, edgeIndices_32Bits: math.IntArrayType): DataTexture {
     ...
     *
     * @param gl
     * @param colors Array of colors for all meshes in the layer
     * @param pickColors Array of pickColors for all meshes in the layer
     * @param vertexBases Array of position-index-bases foteh all meshes in the layer
     ...
     */
    createEachMeshAttributesDataTexture(gl: WebGL2RenderingContext, colors: math.FloatArrayType[], pickColors: math.FloatArrayType[], vertexBases: math.IntArrayType, indexBaseOffsets: math.IntArrayType, edgeIndexBaseOffsets: math.IntArrayType): DataTexture {
        // The number of rows in the texture is the number of meshes in the layer
        ...
        // dataTextureRamStats.sizeDataColorsAndFlags += textureData.byteLength;
        for (let i = 0; i < textureHeight; i++) {
            textureData.set(colors [i], i * 28 + 0); // object color
            textureData.set(pickColors [i], i * 28 + 4); // object pick color
            textureData.set([0, 0, 0, 0], i * 28 + 8);  // object flags
            textureData.set([0, 0, 0, 0], i * 28 + 12); // object flags2
    ...
    /**
     * Creates data texture that holds per-mesh 3D offsets
     *
     * @param gl
     * @param offsets An offset for each mesh
     */
    createEachEdgeOffsetDataTexture(gl: WebGL2RenderingContext, offsets: math.FloatArrayType[]) {
        ...
        // dataTextureRamStats.sizeDataTextureOffsets += textureData.byteLength;
        for (let i = 0; i < offsets.length; i++) {
            textureData.set(offsets [i], i * 3); // object offset
        }
        const texture = gl.createTexture();
        gl.bindTexture(gl.TEXTURE_2D, texture);
     ...
     * @param instanceMatrices Positions instancing matrix for each mesh in the layer. Null if the meshes are not instanced.
     * @param instancesNormalMatrices Normals instancing matrix for each mesh in the layer. Null if the meshes are not instanced.
     */
    createEachMeshMatricesDataTexture(gl: WebGL2RenderingContext, positionDecodeMatrices: math.FloatArrayType, instanceMatrices: math.FloatArrayType, instancesNormalMatrices: math.FloatArrayType): DataTexture {
        ...
        for (let i = 0; i < positionDecodeMatrices.length; i++) {
            ...
            textureData.set(instanceMatrices [i], i * 48 + 16);   // 4x4 values
            textureData.set(instancesNormalMatrices [i], i * 48 + 32);   // 4x4 values
        }
        const texture = gl.createTexture();
        gl.bindTexture(gl.TEXTURE_2D, texture);
    ...
    /**
     * Creates a DataTexture containing the given layer portion IDs.
     * @param gl
     * @param portionIdsArray
     */
    createPointerTableDataTexture(gl: WebGL2RenderingContext, portionIdsArray: math.IntArrayType): DataTexture {
        ...
        // dataTextureRamStats.sizeDataTexturePortionIds += textureData.byteLength;
        const texture = gl.createTexture();
        gl.bindTexture(gl.TEXTURE_2D, texture);
