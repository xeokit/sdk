import {LinearEncoding, sRGBEncoding} from "../../../../../../src/viewer/constants";
import {BasicVBOLayerRenderer} from "../../../BasicVBOLayerRenderer";
import {View, math, constants} from "../../../../../../src/viewer";
...
TEXTURE_DECODE_FUNCS[constants.LinearEncoding] = "linearToLinear";
TEXTURE_DECODE_FUNCS[constants.sRGBEncoding] = "sRGBToLinear";

/**
 * @private
 */
class TrianglesInstancingPBRRenderer extends BasicVBOLayerRenderer {
...
//        }
    ...
    buildFragmentShader(): string {
        const gammaOutput = this.view.gammaOutput; // If set, then it expects that all textures and colors need to be outputted in premultiplied gamma. Default is false.
        const clipping = this.view.sectionPlanesList.length > 0;
        const clippingCaps = false;
        const src = [];
        src.push("#version 300 es");
        ...
        src.push("struct GeometryBucketHandle {");
        ...
        src.push("vec3 BRDF_Specular_GGX(const in IncidentLight incidentLight, const in GeometryBucketHandle geometry, const in vec3 specularColor, const in float roughness) {");
        ...
        src.push("vec3 BRDF_Specular_GGX_Environment(const in GeometryBucketHandle geometry, const in vec3 specularColor, const in float roughness) {");
        src.push("   float dotNV = saturate(dot(geometry.viewNormal, geometry.viewEyeDir));");
        ...
        if (lightsState.lightMaps.length > 0 || lightsState.reflectionMaps.length > 0) {
            ...
            src.push("void computePBRLightMapping(const in GeometryBucketHandle geometry, const in Material material, inout ReflectedLight reflectedLight) {");
        ...
        src.push("void computePBRLighting(const in IncidentLight incidentLight, const in GeometryBucketHandle geometry, const in Material material, inout ReflectedLight reflectedLight) {");
        src.push("   float dotNL     = saturate(dot(geometry.viewNormal, incidentLight.direction));");
        ...
        src.push("GeometryBucketHandle       geometry;");
