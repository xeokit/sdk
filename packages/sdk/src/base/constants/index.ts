 ...
 * ## Categories
 *
 * - **Texture wrapping** — `RepeatWrapping`, `ClampToEdgeWrapping`, `MirroredRepeatWrapping`.
 * - **Texture filter** — `NearestFilter`, `LinearFilter`, and the `*MipMap*` variants.
 * - **Texture format / encoding** — `RGBAFormat`, `RGBA_S3TC_DXT*_Format`, `sRGBEncoding`, `LinearEncoding`.
 * - **Primitive types** — `TrianglesPrimitive`, `LinesPrimitive`, `PointsPrimitive`, `SolidPrimitive`, `SurfacePrimitive`.
 * - **Media types** — `PNGMediaType`, `JPEGMediaType`, `KTX2MediaType`, etc.
 * - **Projection / navigation** — `PerspectiveProjectionType`, `OrthoProjectionType`, `OrbitNavigationMode`, etc.
 * - **Key codes + layouts** — `KEY_A`..`KEY_Z`, `KEY_NUM_*`, `QWERTYLayout`, `AZERTYLayout`.
 *
 * <br>
 *
 * ## Features
...
/**
 * Indicates that a {@link model!scene.SceneGeometry | SceneGeometry} contains 3D Gaussian Splats — oriented,
