 ...
 *
 * `tEnter` and `tExit` are the parametric distances along the ray at which it
 * enters and exits the object's world-space AABB. The hit point is
 * `origin + dir * tEnter`. Results are reported in ray-AABB granularity —
 * triangle-level intersection tests are not performed.
 */
...
/**
 * Visitor callback. Return `false` to stop iteration early; any other return
 * value continues.
 */
...
interface BVHLeaf {
  objectId: string;
  /** Reference into {@link SceneAABB3Index}'s cached object AABB. */
  aabb: AABB3Float;
...
export class SceneCollisionIndex {
  ...
  constructor(scene: Scene) {
    ...
    this.#unsubscribers.push(
      scene.events.onSceneObjectCreated.subscribe(markDirty),
      scene.events.onSceneObjectDestroyed.subscribe(markDirty),
      scene.events.onSceneMeshMoved.subscribe(markDirty),
      ...
      scene.events.onSceneMeshMatrixChanged.subscribe(markDirty),
      scene.events.onSceneModelDestroyed.subscribe(markDirty)
    );
  }
  ...
  /**
   * Tears down event subscriptions and releases the BVH.
   */
  destroy(): void {
...
/**
 * Gets or lazily creates the shared {@link SceneCollisionIndex} for a Scene.
 *
 * Mirrors the {@link getSceneAABB3Index} pattern: one index per Scene,
 * destroyed automatically when the Scene fires `onSceneDestroyed`.
 */
