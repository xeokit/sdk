...
import {createMat4Float64, inverseMat4, transformPoint3, transformVec3} from "../../math/matrix";
...
export interface SceneTriangleHit {
  /** ID of the {@link scene!SceneObject | SceneObject} the triangle belongs to. */
  objectId: string;
  /** ID of the {@link scene!SceneMesh | SceneMesh} the triangle belongs to. */
  meshId: string;
...
export interface SceneRayTriangleOptions {
   ...
   * object before any per-mesh / per-triangle work — the right place to
   * drop hidden, non-pickable, or marker-only objects without paying for
   * their triangle scan.
   */
