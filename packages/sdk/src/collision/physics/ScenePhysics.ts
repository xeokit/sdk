...
import {createVec3Float64, createVec4Float64,  type Vec3Float, type Vec3} from "../../math/vector";
...
export class ScenePhysics {
  ...
  /**
   * Constructs a new ScenePhysics for the given {@link Scene}.
 ...
 * @param scene
 */
export function getScenePhysics(scene: Scene) {
  let sceneIndex = sceneIndexes[scene.id];
  if (!sceneIndex) {
    sceneIndex = sceneIndexes[scene.id] = new ScenePhysics(scene);
