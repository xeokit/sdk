...
export class ModelConverter {
   ...
   *
   * @param pipelineRequest - The parameters specifying the pipeline and input data.
   ...
   */
  convert(pipelineRequest: ModelConverterRequest): Promise<ModelConverterResult> {
    ...
    return new Promise((resolve, reject) => {
      ...
      const pipelineId = pipelineRequest.pipeline;
      if (!pipelineId) {
      ...
      const conversionParamsInputs = pipelineRequest.inputs;
      ...
      const pipelineResult: ModelConverterResult = {
        modelConverter: this,
        pipeline: pipelineId,
        scene,
        data,
      ...
      const requestSignal = pipelineRequest.signal;
      const requestYieldIntervalMs = pipelineRequest.yieldIntervalMs;
        ...
        for (const pipelineInputId of pipelineInputIds) {
          ...
          const loadFileData = async (fileData) => {
            ...
            try {
              ...
              pipelineResult.inputs[pipelineInputId] = {
                filePath,
                fileData,
              ...
              pipelineResult.errors.push(errorMsg);
              pipelineResult.inputs[pipelineInputId] = {
                fileData,
                fileDataType: loader.fileDataType,
                fileDataSizeBytes,
      ...
      const processOutputs = async () => {
        ...
        for (const pipelineOutputId of pipelineOutputIds) {
          ...
          try {
            ...
            pipelineResult.outputs[pipelineOutputId] = {
              fileData,
      ...
      const runInspection = async (
        ...
        pipelineResult.inspection = {bySceneModel: {}};
        let aborted = false;
        ...
        for (const sceneModelId of Object.keys(scene.models)) {
          ...
          pipelineResult.inspection.bySceneModel[sceneModelId] = {
            sceneModel: sceneModelId,
            report,
            fixResult,
            reReport,
        ...
        if (dataModelIds.length > 0) {
          pipelineResult.inspection.byDataModel = {};
          for (const dataModelId of dataModelIds) {
            ...
            pipelineResult.inspection.byDataModel[dataModelId] = {
              dataModel: dataModelId,
              report: dataReport,
              durationMs: Date.now() - t0,
            };
      ...
      const runPipeline = async (): Promise<ModelConverterResult> => {
        ...
        if (blocked) return pipelineResult;
