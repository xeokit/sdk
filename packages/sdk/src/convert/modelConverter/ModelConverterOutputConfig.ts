...
export interface ModelConverterOutputConfig {
  ...
  /**
   * The key of an {@link formats!ModelExporter | ModelExporter} declared in
  ...
  /**
   * Optional schema version to be used by the {@link formats!ModelExporter | ModelExporter}
   * when generating the output file.
   */
  version?: string;

  /**
   * Optional ID of the {@link model!scene.SceneModel | SceneModel} that provides the
   * scene data for export. Must reference a SceneModel defined in a pipeline input.
   */
  sceneModel?: string;

  /**
   * Optional ID of the {@link model!data.DataModel | DataModel} that provides the
