 ...
 *
 * **Schema-aware validator for {@link data!DataModel | DataModel}.**
 *
 ...
 *
 * Same shape as {@link sceneModelInspector | @xeokit/sdk/sceneModelInspector}
 * — pluggable inspections + a default registry — minus the
 * auto-fix half. **No fixes ship for DataModel issues**: SDK policy
 * is to surface defects in user data rather than silently rewrite
 * it. Triage is human work. Unlike geometry — where certain
 * glitches (degenerate triangles, duplicate vertices) are clearly
 * mechanical artefacts safe to clean up automatically — defects in
 * the data graph almost always reflect intentional or
 * authoring-decision content the application owner needs to see
 * and decide on.
 *
 * ## Installation
 *
 ...
 *             │
 *             ▼
 * inspectDataModel ──► InspectionReport
 *      ▲                { issues, errors, warnings, info, byCode }
 *      │
 *      DataModel
 * ```
 *
 ...
 *
 * ```ts
 * import {DEFAULT_INSPECTION_REGISTRY} from "@xeokit/sdk/dataModelInspector";
 *
