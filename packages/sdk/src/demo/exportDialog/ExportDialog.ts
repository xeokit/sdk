...
.xkt-exp-dialog .xkt-exp-row input[type="checkbox"] {
  appearance: auto;
  width: 14px;
  height: 14px;
  margin: 0;
...
export class ExportDialog extends FloatingPanelBase {
  ...
  private readonly _selectedModelIds = new Set<string>();
   ...
   */
  private readonly _datasetParamValues: Record<string, Record<string, any>> = {};
  ...
  get visible(): boolean {
    return this._dialog.style.display !== "none";
  }
  ...
  protected _buildDom(): void {
    ...
    this._dialog = el("div", "xkt-exp-dialog");
    ...
    title.innerHTML =
      ...
      `<span class="xkt-exp-title-text">Export</span>`;
    ...
    // ── Models section
    {
      ...
      const label = el("span", "xkt-exp-section-label", {textContent: "Models"});
      const bulk = el("div", "xkt-exp-bulk");
      const allBtn = el("button", "xkt-exp-bulk-btn", {type: "button", textContent: "All"}) as HTMLButtonElement;
      const noneBtn = el("button", "xkt-exp-bulk-btn", {type: "button", textContent: "None"}) as HTMLButtonElement;
      allBtn.addEventListener("click", () => {
        this._selectedModelIds.clear();
        for (const id of Object.keys(this.demoHelper.scene.models || {})) this._selectedModelIds.add(id);
        for (const id of Object.keys(this.demoHelper.data.models  || {})) this._selectedModelIds.add(id);
        this._renderModels();
    ...
    this._dialog.appendChild(this._bodyEl);
    ...
    this._dialog.appendChild(this._progressEl);
  ...
  private _scheduleRefresh(): void {
    ...
    requestAnimationFrame(() => {
      this._refreshScheduled = false;
      if (this._destroyed || !this._listenersAttached) return;
      // Prune selections that no longer match a live SceneModel
      // or DataModel — destruction can drop a row out from under
      // the user's checkbox.
      const live = new Set<string>();
      for (const id of Object.keys(this.demoHelper.scene.models || {})) live.add(id);
      for (const id of Object.keys(this.demoHelper.data.models  || {})) live.add(id);
      for (const id of Array.from(this._selectedModelIds)) {
        if (!live.has(id)) this._selectedModelIds.delete(id);
      }
      this._renderModels();
    });
  }
  ...
  private _renderModels(): void {
    ...
    // Iterate the union of SceneModel + DataModel ids so a
    // DataModel that has no matching SceneModel still shows up
    // (only the `datamodel` dataset can target it, but skipping
    // it from the picker would be confusing).
    ...
    for (const id of ids) {
      ...
      const row = el("label", "xkt-exp-row");
      const cb = el("input", undefined, {type: "checkbox"}) as HTMLInputElement;
      cb.checked = this._selectedModelIds.has(id);
      cb.addEventListener("change", () => {
        if (cb.checked) this._selectedModelIds.add(id);
        else            this._selectedModelIds.delete(id);
        this._refreshExportEnabled();
      });
      row.appendChild(cb);
  ...
  private _renderDatasets(): void {
    ...
    for (const desc of DEFAULT_DATASET_TYPES) {
      ...
      // Skip dataset descriptors with any unknown format id — keeps
      // the registry in lockstep with the offered list.
      if (!ids.every(i => FORMAT_REGISTRY[i])) continue;

      const chip = el("button", "xkt-exp-chip", {
        type:  "button",
        title: desc,                  // technical descriptor on hover
      }) as HTMLButtonElement;
  ...
  private _refreshExportEnabled(): void {
    const ok = this._selectedModelIds.size > 0 && this._selectedDatasets.size > 0;
  ...
  async runExport(): Promise<void> {
    const selectedIds = Array.from(this._selectedModelIds);
    if (selectedIds.length === 0) {
      this._setStatus("No models selected.", "error");
      return;
    }
