...
import {BVHPickStrategy, type PickStrategy} from "../../../picking";
...
import type {AngleMeasurementsToolParams} from "./AngleMeasurementsToolParams";
import {MouseAngleMeasurementsControl} from "./MouseAngleMeasurementsControl";

const SVG_NS = "http://www.w3.org/2000/svg";

/**
 * AngleMeasurementsTool — overlay widget that renders one or more
 * {@link AngleMeasurement | AngleMeasurements} on a {@link View}'s
 * canvas.
 *
 ...
 * {@link MouseAngleMeasurementsControl}.
 *
 * ### Lifecycle
 *
 ...
 */
export class AngleMeasurementsTool {

  /** Per-View instance registry. */
  private static readonly _instances = new WeakMap<View, AngleMeasurementsTool>();
   ...
   * {@link AngleMeasurementsToolParams.view}, or constructs one.
   * If the existing instance was hidden it's shown again.
   */
  static openFor(params: AngleMeasurementsToolParams): AngleMeasurementsTool {
    let inst = AngleMeasurementsTool._instances.get(params.view);
    if (inst && !inst._destroyed) {
      inst.show();
      return inst;
    }
    inst = new AngleMeasurementsTool(params);
    return inst;
  }
  ...
  readonly measurements: { [id: string]: AngleMeasurement } = {};
   ...
   */
  readonly onMeasurementsChanged: EventEmitter<AngleMeasurementsTool, void>;
  ...
  // Lazy mouse control — only built when callers ask for it.
  private _mouseControl: MouseAngleMeasurementsControl | null = null;
   ...
   * same View.
   */
  constructor(params: AngleMeasurementsToolParams) {
    if (!params || !params.view) {
      throw new Error("[AngleMeasurementsTool] params.view is required");
    }
    ...
    this.onMeasurementsChanged = new EventEmitter(
      new EventDispatcher<AngleMeasurementsTool, void>(),
    );
    ...
    const prior = AngleMeasurementsTool._instances.get(this.view);
    if (prior && prior !== this && !prior._destroyed) prior.destroy();
    AngleMeasurementsTool._instances.set(this.view, this);
  }
   ...
   */
  createMeasurement(params: AngleMeasurementParams): AngleMeasurement {
    if (this._destroyed) {
      throw new Error("[AngleMeasurementsTool] tool is destroyed");
    }
    const id = params.id ?? `am-${this._nextId++}`;
    if (this.measurements[id]) {
      throw new Error(`[AngleMeasurementsTool] measurement id already exists: ${id}`);
    }
    const m = new AngleMeasurement(
      id,
  ...
  /**
   * Returns the lazily-built {@link MouseAngleMeasurementsControl}
   * bound to this tool. The control is *inactive* until
   * {@link MouseAngleMeasurementsControl.activate | activate()}
   * is called.
   */
  get mouseControl(): MouseAngleMeasurementsControl {
    if (!this._mouseControl) {
      this._mouseControl = new MouseAngleMeasurementsControl(this);
    }
  ...
  destroy(): void {
    ...
    if (AngleMeasurementsTool._instances.get(this.view) === this) {
      AngleMeasurementsTool._instances.delete(this.view);
    }
  }
  ...
  private _updateOne(m: AngleMeasurement): void {
