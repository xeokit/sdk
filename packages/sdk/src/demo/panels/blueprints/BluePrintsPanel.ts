 ...
 * wireframe-projection "blueprint" SceneModels for every other
...
import {
  ...
  clearBlueprint,
  projectBlueprint,
  type BlueprintProjectionFace,
} from "../../systems/blueprintProjector/projectBlueprint";
...
export interface BluePrintsPanelParams {
  /**
   * DemoHelper whose Scene the panel manages blueprints for.
   * Doubles as the WeakMap key for {@link BluePrintsPanel.openFor}
...
interface ViewSpec {
  ...
  face: BlueprintProjectionFace;
  /** Whether this view is enabled by default when blueprints are first turned on. */
  defaultOn: boolean;
}
...
interface BlueprintConfig {
   ...
   * the plane is dropped from the blueprint.
   */
  clipEnabled: boolean;
  /**
...
.xkt-bp-panel .xkt-bp-row[data-disabled="1"] .xkt-bp-row-detail {
  /* Visual cue only — controls stay interactive so the user
     can configure a blueprint *before* toggling "Generate" on. */
  opacity: 0.65;
}
`;
...
export class BluePrintsPanel extends FloatingPanelBase {
   ...
   * alive for GC.
   */
  private static readonly _instances = new WeakMap<DemoHelper, BluePrintsPanel>();

  /**
   * SVG markup for the panel's title-bar glyph — three stacked
   * rectangles suggesting a folded blueprint sheet. Strokes use
   * `currentColor`.
   */
  ...
  static getFor(demoHelper: DemoHelper): BluePrintsPanel | undefined {
    const inst = BluePrintsPanel._instances.get(demoHelper);
    return inst && !inst._destroyed ? inst : undefined;
  }

  /**
   * Returns `true` when `modelId` matches the panel's blueprint
   * target-id pattern (`{srcId}__bp_{viewLabel}`). Used to filter
   * candidate source SceneModels — blueprints, including ones
   * left over from a previous panel instance or produced
   * directly through `projectBlueprint(...)` with the panel's
   * id convention, never reappear as something the user can
   * project from.
   */
  static isBlueprintModelId(modelId: string): boolean {
    return BLUEPRINT_ID_PATTERN.test(modelId);
  }

  static openFor(params: BluePrintsPanelParams): BluePrintsPanel {
    let inst = BluePrintsPanel._instances.get(params.demoHelper);
    if (inst && !inst._destroyed) {
      inst.show();
      return inst;
    }
    inst = new BluePrintsPanel(params);
    return inst;
  }
  ...
  private readonly _configs = new Map<string, BlueprintConfig>();

  /**
   * Set of projection-SceneModel ids the panel has produced. The
   * onSceneModelCreated subscription filters these out so the
   * panel never tries to project blueprints of its own
   * blueprints. The set also bounds {@link clearAll} on destroy
  ...
  constructor(params: BluePrintsPanelParams) {
    if (!params?.demoHelper) {
      throw new Error("BluePrintsPanel: demoHelper is required");
    }
    super({
    ...
    const prior = BluePrintsPanel._instances.get(this.demoHelper);
    if (prior && !prior._destroyed) prior.destroy();
    BluePrintsPanel._instances.set(this.demoHelper, this);
  ...
  destroy(): void {
    ...
    // closing the panel shouldn't leave orphan blueprints
    // hanging around the Scene.
    for (const id of this._ownedProjectionIds) {
      clearBlueprint(this.demoHelper.scene, id);
    }
    ...
    if (BluePrintsPanel._instances.get(this.demoHelper) === this) {
      BluePrintsPanel._instances.delete(this.demoHelper);
    }
    super.destroy();
  }
  ...
  protected _buildDom(): void {
    this._pill = el("button", "xkt-bp-pill", {
      type: "button",
      title: "Reopen Blueprints",
      hidden: true,
      textContent: "Blueprints",
    ...
    title.innerHTML =
      `<span class="xkt-bp-title-icon">${BluePrintsPanel.iconSvg()}</span>` +
      `<span class="xkt-bp-title-stack">` +
        `<span class="xkt-bp-title-text">Blueprints</span>` +
  ...
  private _attachSubscriptions(): void {
    ...
    if (sceneEv.onSceneModelCreated?.subscribe) {
      sub(sceneEv.onSceneModelCreated.subscribe((_: unknown, model: SceneModel) => {
        ...
        if (BluePrintsPanel.isBlueprintModelId(model.id)) return;
        this._renderBody();
      }));
    }
    if (sceneEv.onSceneModelDestroyed?.subscribe) {
      sub(sceneEv.onSceneModelDestroyed.subscribe((_: unknown, model: SceneModel) => {
        ...
        if (BluePrintsPanel.isBlueprintModelId(model.id)) {
          // External blueprint vanished — nothing for the panel
  ...
  private _listSourceModels(): SceneModel[] {
    ...
    for (const id of Object.keys(models)) {
      ...
      // Skip SceneModels that match the panel's own blueprint
      // id pattern, regardless of whether *this* panel instance
      // created them. Covers blueprints left over from a prior
      // panel session or generated by a direct call to
      // `projectBlueprint(...)` with the panel's id convention.
      if (BluePrintsPanel.isBlueprintModelId(id)) continue;
      out.push(models[id]);
    }
    return out;
  }
  ...
  private _getOrInitConfig(modelId: string): BlueprintConfig {
  ...
  private async _apply(modelId: string): Promise<void> {
    ...
    // Without this guard, projectBlueprint would build and
    ...
    if (!canProjectBlueprint(sourceModel, requiredMode)) return;
    ...
    for (const spec of this._viewSpecs) {
      ...
      // — never both — to keep the blueprint look consistent
      // per source model.
      ...
      // Eagerly claim the target id so a concurrent teardown
      // (e.g. the user toggling "Generate" off while an earlier
      // _apply is still streaming objects in) destroys our
      // in-flight target via `clearBlueprint`. The projector
      ...
      const result = await projectBlueprint({
        scene:         this.demoHelper.scene,
        sourceModel,
        ...
        // "blueprints" ViewLayer so the default layer stays
        // clear for the source SceneModels the panel projects.
        layerId: "blueprints",
        // Yield to the renderer between batches so the blueprint
      ...
      if (result.ok === false) {
        ...
        // sweep stale ids via the no-op `clearBlueprint` path.
        if (!result.error.includes("destroyed mid-projection")) {
          console.warn(`[BluePrintsPanel] projectBlueprint(${targetId}) failed:`, result.error);
        }
        continue;
      }
    }
  ...
  private _teardown(modelId: string): void {
    for (const spec of this._viewSpecs) {
      ...
      clearBlueprint(this.demoHelper.scene, targetId);
 ...
 * BluePrintsPanel.isBlueprintModelId} to filter blueprints out
 * of the candidate source list, regardless of which panel
 * instance created them.
 */
const BLUEPRINT_ID_PATTERN = /__bp_[a-z0-9_]+$/;
