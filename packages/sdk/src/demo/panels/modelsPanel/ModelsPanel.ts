...
export class ModelsPanel extends FloatingPanelBase {
  ...
  private static readonly _instances = new WeakMap<DemoHelper, ModelsPanel>();
  ...
  static getFor(demoHelper: DemoHelper): ModelsPanel | undefined {
    const inst = ModelsPanel._instances.get(demoHelper);
    return inst && !inst._destroyed ? inst : undefined;
  }

  static openFor(params: ModelsPanelParams): ModelsPanel {
    let inst = ModelsPanel._instances.get(params.demoHelper);
  ...
  constructor(params: ModelsPanelParams) {
    if (!params?.demoHelper) {
      throw new Error("ModelsPanel: demoHelper is required");
    }
    super({
    ...
    const prior = ModelsPanel._instances.get(this.demoHelper);
    if (prior && !prior._destroyed) prior.destroy();
    ModelsPanel._instances.set(this.demoHelper, this);
  ...
  destroy(): void {
    ...
    if (ModelsPanel._instances.get(this.demoHelper) === this) {
      ModelsPanel._instances.delete(this.demoHelper);
    }
    super.destroy();
  }
  ...
  private _attachSceneSubscriptions(): void {
    const ev = (this.demoHelper.scene as any).events;
    if (!ev) return;
  ...
  private _renderModelSection(sceneModel: SceneModel): HTMLElement {
    ...
    const dataModel  = (this.demoHelper.data as any).models?.[id];
    const origin     = this.demoHelper.getModelOrigin(id);
    ...
    body.append(
      ...
      this._group("Open in", this._buttonRow([
        ["Scene Health", () => this.demoHelper.getSceneHealthPanel(sceneModel)],
        ["Data Health",  () => this.demoHelper.getDataHealthPanel(dataModel ?? undefined)],
        ["Explorer",     () => this.demoHelper.getExplorer()],
        ["Boundaries",   () => this.demoHelper.openBoundariesPanel()],
      ])),
    );
  ...
  private _setVisible(sceneModel: SceneModel, visible: boolean): void {
    const view = this.demoHelper.viewer?.viewList?.[0];
    if (!view) return;
  ...
  private _unload(modelId: string): void {
    this.demoHelper.destroyModel(modelId);
  }

  private _cameraFlight(): any {
    const view = this.demoHelper.viewer?.viewList?.[0];
    if (!view) return null;
    return this.demoHelper.views?.[view.id]?.cameraFlight ?? null;
  }

  private _modelAabb(sceneModel: SceneModel): number[] | null {
    const idx: any = (this.demoHelper as any).collisionIndex
      ?? getSceneCollisionIndex(this.demoHelper.scene as any);
  ...
  private _loadedSceneModels(): SceneModel[] {
    const models = (this.demoHelper.scene as any)?.models as Record<string, SceneModel> | undefined;
    if (!models) return [];
    return Object.values(models).filter(m => !(m as any).destroyed);
  }
}
