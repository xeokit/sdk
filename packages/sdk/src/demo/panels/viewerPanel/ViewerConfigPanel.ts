...
export class ViewerConfigPanel extends FloatingPanelBase {
  ...
  /**
   * Optional DemoHelper — when wired, "New View" and the per-View
   ...
   * View (clearing their auto-canvases through {@link DemoHelper})
   * and recreate each one from the loaded params. Routing through
   * the DemoHelper is what keeps the canvas DOM in sync — the
   * Viewer itself expects canvases to exist before each View is
   * created, and DemoHelper auto-creates them when no explicit
  ...
  private _applyLoadedParams(params: ViewerParams): void {
    ...
    if (helper) {
      for (const viewParams of views) {
        ...
        // serialisation in name only — DemoHelper.createView
