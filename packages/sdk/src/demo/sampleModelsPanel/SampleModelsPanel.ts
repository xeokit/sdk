 ...
 * "Load" button calls `demoHelper.loadDataset` with the modelId
 ...
 *
 * ```ts
 * import {SampleModelsPanel} from "@xeokit/sdk/demo";
 *
 * const cat = new SampleModelsPanel({demoHelper});
 * cat.show();
 * cat.hide();
 * cat.destroy();
 * ```
...
export interface SampleModelsPanelParams {
  ...
  /** DemoHelper that owns the Scene + Data the catalog loads into. */
  demoHelper: DemoHelper;

  /**
   * URL of the catalog JSON file. Defaults to
   * `${demoHelper.modelsDir}/index.json` (typically
...
export class SampleModelsPanel extends FloatingPanelBase {
   ...
   * independent panels.
   */
  private static readonly _instances = new WeakMap<DemoHelper, SampleModelsPanel>();

  static getFor(demoHelper: DemoHelper): SampleModelsPanel | undefined {
    const inst = SampleModelsPanel._instances.get(demoHelper);
    return inst && !inst._destroyed ? inst : undefined;
  }

  /**
   * Reveal (or lazily mount) a panel for `params.demoHelper`.
   * Idempotent.
   */
  static openFor(params: SampleModelsPanelParams): SampleModelsPanel {
    let inst = SampleModelsPanel._instances.get(params.demoHelper);
  ...
  /**
   * SVG markup for the panel's title-bar glyph (a stack of
   * model rows, evoking a list / catalog). Strokes use
   * `currentColor` — matches the other panels' icon contract.
   */
  static iconSvg(): string {
    return `<svg viewBox="0 0 24 24" aria-hidden="true">` +
      ...
      `<line x1="7"  y1="12.5" x2="17" y2="12.5" stroke="currentColor" stroke-width="1.6" stroke-linecap="round"/>` +
      `<line x1="7"  y1="16"  x2="13" y2="16"  stroke="currentColor" stroke-width="1.6" stroke-linecap="round"/>` +
    `</svg>`;
  }
  ...
  private _addModeCheckbox!: HTMLInputElement;
  /**
  ...
  constructor(params: SampleModelsPanelParams) {
    ...
    // Replace any prior panel bound to the same DemoHelper —
    // keeps openFor's idempotence honest and clears stale DOM
    // from a hot-reload / re-import.
    const prior = SampleModelsPanel._instances.get(this.demoHelper);
    if (prior && !prior._destroyed) prior.destroy();
    SampleModelsPanel._instances.set(this.demoHelper, this);
  ...
  private _attachSceneListeners(): void {
    const scene: any = (this.demoHelper as any).scene;
  ...
  private _renderSampleModels(): void {
    ...
    // Re-rendering the catalog blows away every row DOM node, so
    // the per-row load/unload state tracked against those nodes
    // has to be discarded too. (In practice the catalog is built
    // once on first show — but a stale registry would silently
    // mis-track buttons after a hot-reload-style re-render.)
    this._rows.length = 0;
    ...
    for (const id of ids) {
      ...
      this._bodyEl.appendChild(this._renderModelSection(id, entry));
    }

    // Reflect any models that are *already* loaded into the
    // helper at the time the catalog renders — covers the case
    // where the panel is opened after a model was loaded by
    // some other mechanism (sample-models URL param, code, etc).
    this._reconcileWithLoadedScene();
  }

  private _renderModelSection(id: string, entry: SampleModelEntry): HTMLElement {
    const section = el("details", "xkt-sam-section") as HTMLDetailsElement;
    const summary = el("summary");
    const label = el("span", "xkt-sam-section-label", {textContent: id});
    const datasets = entry.dataset || [];
    const meta = el("span", "xkt-sam-section-meta", {
      textContent: `${datasets.length} dataset${datasets.length === 1 ? "" : "s"}`,
    });
    summary.append(label, meta);
    section.appendChild(summary);

    const body = el("div", "xkt-sam-section-body");

    // Coord-system summary line — a one-liner so the user can
    // tell Y-up from Z-up at a glance without unfolding more
    // detail.
    if (entry.coordSys) {
      ...
      if (parts.length > 0) {
        const coord = el("div", "xkt-sam-coord", {textContent: parts.join(" · ")});
  ...
  private async _runLoad(
    ...
    try {
      const result = await (this.demoHelper as any).loadDataset({
        modelId,
        formats,
        clear: !additive,
      });
   ...
   * `DemoHelper.destroyModel` so both the SceneModel and any
  ...
  private _runUnload(entry: RowEntry): void {
    ...
    if (!id) return;
    try {
      (this.demoHelper as any).destroyModel(id);
    } catch (e: any) {
    ...
    if ((this.demoHelper as any).scene?.models?.[id] === undefined) {
  ...
  private _reconcileWithLoadedScene(): void {
    const sceneModels: Record<string, any> = (this.demoHelper as any).scene?.models || {};
  ...
  protected _buildDom(): void {
    ...
    title.innerHTML =
      ...
      `<span class="xkt-sam-title-text">Sample Models</span>` +
    ...
    // Controls strip — currently a single "Add to scene" toggle
    // that flips `loadDataset` between replace (default) and
    // additive ("load alongside") behaviour. Its state mutates
    // every Load button's label so the click is unambiguous.
    const controlsRow = el("div", "xkt-sam-controls");
    const addLabel = el("label", "xkt-sam-add-toggle", {
      title: "When on, clicking Add loads the dataset alongside any models already in the scene.",
    });
    this._addModeCheckbox = el("input", undefined, {type: "checkbox"}) as HTMLInputElement;
    this._addModeCheckbox.checked = this._addMode;
    this._addModeCheckbox.addEventListener("change", () => {
      this._addMode = this._addModeCheckbox.checked;
      // Toggling Add mode doesn't change which rows are loaded,
      // but a refresh re-derives the "Unload existing first"
      // tooltips correctly.
      this._refreshButtonStates();
    });
    addLabel.append(
      this._addModeCheckbox,
      el("span", "xkt-sam-add-toggle-text", {textContent: "Add to scene"}),
      el("span", "xkt-sam-add-toggle-hint", {
        textContent: "(keep existing models)",
      }),
    );
    controlsRow.appendChild(addLabel);
    this._panel.appendChild(controlsRow);
