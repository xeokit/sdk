...
export interface SceneHealthPanelParams {
   ...
   * the first SceneModel in the Scene is selected. Also the
   * legacy single-model entry point: passing only this is
   * equivalent to passing `{scene: sceneModel.scene,
   * focusSceneModel: sceneModel}`.
   */
  focusSceneModel?: SceneModel;

  /**
   * Backwards-compatible alias for {@link focusSceneModel}. Older
   * callers that pass `{sceneModel}` keep working.
   *
   * @deprecated Prefer `focusSceneModel`.
   */
  ...
  /**
   * DemoHelper instance. Used by Locate to read the SDK's
...
export class SceneHealthPanel extends FloatingPanelBase {
   ...
   * backwards compatibility with the per-SceneModel caller
   * pattern that predates the Scene-scoped refactor.
   */
  ...
  static iconSvg(): string {
    return `<svg viewBox="0 0 24 24" aria-hidden="true">` +
      ...
      `<path d="M7 12.2 L10.5 15.7 L17 8.5" ` +
  ...
  private _wireEvents(): void {
    ...
    this._clearLocateBtn.addEventListener("click", () => {
      this._clearLocate();
      if (this._demoHelper && this._view) this._demoHelper.viewFit(this._view);
    });
  ...
  private _locate(objectIds: string[]): void {
    ...
    if (!this._demoHelper) return;
    const idx = this._demoHelper.collisionIndex;
    ...
    if (union) {
      const cf = this._demoHelper.views[view.id].cameraFlight;
 ...
 * for stable tab order across refreshes. Cheap (small N).
 */
