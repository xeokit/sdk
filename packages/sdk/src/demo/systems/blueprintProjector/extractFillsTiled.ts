/**
 * Tiled, BVH-narrowed fill-polygon extraction for the blueprint
 * projector.
 *
 ...
 *
 * @module demo/systems/blueprintProjector/extractFillsTiled
 */
...
export interface ExtractFillsTiledParams {
   ...
   * wants a single atomic blueprint with no intermediate
   * frames.
   *
...
export async function extractFillsTiled(params: ExtractFillsTiledParams): Promise<FillPolygons[]> {
  ...
  for (let ty = 0; ty < tilesY; ty++) {
    for (let tx = 0; tx < tilesX; tx++) {
      ...
      // blueprint paints in as extraction streams through.
