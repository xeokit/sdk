 ...
 * rotation quaternion, returning the 6 unique values of the symmetric 3×3
 * matrix (`[σxx, σxy, σxz, σyy, σyz, σzz]`) ready to upload to the GPU.
 *
 * Σ = Mᵀ·M with M = diag(scale)·R, where R is the rotation matrix of the
 * (normalised) quaternion. This composition was validated in the standalone
 * spike against a reference `.splat` scene — using R·S / M·Mᵀ instead builds
 * the transpose, which mis-orients every splat into "fur".
 *
 * @param scale - Per-axis std-devs `[sx, sy, sz]` (linear, already exp()-ed).
 * @param quat  - Rotation quaternion in `[x, y, z, w]` order.
 */
export function computeCovariance3D(
  ...
  // Σ = Mᵀ·M (column dot-products of M).
