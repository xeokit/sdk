 ...
 *   - bytes 28..31  rotation   : 4 × uint8     (quaternion, decoded (b-128)/128)
 *
...
/** Decoded splat buffers, ready to pack into GPU data textures. */
export interface ParsedSplats {
  /** Number of splats. */
  count: number;
  /** `count × 3` packed positions (xyz). */
  positions: Float32Array;
  /** `count × 3` packed per-axis scales (xyz). */
  scales: Float32Array;
  /** `count × 4` packed RGBA colour bytes (0..255). */
  colors: Uint8Array;
  /** `count × 4` packed rotation quaternions (xyzw), decoded to ~[-1,1]. */
 ...
 *
 * @param data - The raw `.splat` file bytes.
 * @throws if the buffer length is not a whole number of 32-byte records.
 */
export function parseSplat(data: ArrayBuffer | Uint8Array): ParsedSplats {
  ...
  for (let i = 0; i < count; i++) {
    ...
    // (the renderer normalises before building the covariance).
