...
import type {TiledXKTData} from "./TiledXKTData";

const MESH_ATTRIBUTES = 6; // RGB, opacity, metallic, roughness

/**
 * Decode a tiled XKT payload (V7 onwards) into a SceneModel (and optionally a
 * DataModel).
 *
 ...
 *
 * The container, compression and metadata representation differ across
 * versions; those differences are resolved by each version's parser into the
 * common {@link TiledXKTData}, leaving this build step version-agnostic.
 *
 * @private
 */
export async function buildTiledSceneModel(params: {
  xktData: TiledXKTData,
  ...
  const {
    positions, colors, colorComponents, indices,
        ...
        if (valid === undefined) {
          ...
          if (primitive === PointsPrimitive) {
            ...
            if (colSlice.length > 0) {
              geometryParams.colorsCompressed = colorComponents === 3 ? rgbToRgba(colSlice) : colSlice;
            }
          } else {
            ...
            geometryParams.indices = eachGeometryPrimitiveType[geometryIndex] === 4
              ...
              : indSlice;
            // These versions store no usable vertex normals for triangle
            // geometry, which is faceted (architectural). Leaving normals off
            // lets the renderer derive per-face flat normals — averaging
            // shared-vertex normals would smear shading across hard edges.
          }
...
/** Expand RGB vertex colours (V7–V8) to the RGBA the SceneModel expects. */
function rgbToRgba(rgb: Uint8Array): Uint8Array {
  const out = new Uint8Array((rgb.length / 3) * 4);
  for (let i = 0, j = 0; i < rgb.length; i += 3) {
    out[j++] = rgb[i];
    out[j++] = rgb[i + 1];
    out[j++] = rgb[i + 2];
    out[j++] = 255;
