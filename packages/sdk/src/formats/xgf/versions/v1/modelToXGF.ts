...
import {createMat4Float64, isIdentityMat4, mulMat4} from "../../../../base/math/matrix";
...
export function modelToXGF(params: {
  ...
  const xgfData = {
    ...
    aabbs: null,
    eachGeometryPositionsBase: new Uint32Array(numGeometries), // For each geometry, the index of its first element in xgfData.positions. Every primitive type has positions.
    eachGeometryColorsBase: new Uint32Array(numGeometries), // For each geometry, the index of its first element in xgfData.colors. If the next geometry has the same index, then this geometry has no colors.
    eachGeometryIndicesBase: new Uint32Array(numGeometries), // For each geometry, the index of its first element in xgfData.indices. If the next geometry has the same index, then this geometry has no indices.
    eachGeometryEdgeIndicesBase: new Uint32Array(numGeometries), // For each geometry, the index of its first element in xgfData.edgeIndices. If the next geometry has the same index, then this geometry has no edge indices.
