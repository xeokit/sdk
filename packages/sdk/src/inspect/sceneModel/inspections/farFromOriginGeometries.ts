 ...
 * Flags every {@link model!scene.SceneGeometry | SceneGeometry} whose **local** AABB centroid
 * has magnitude above
...
export const farFromOriginGeometries: Inspection = {
  ...
  config: {
    enabled: {
      kind: "boolean",
      key: "checkGeometryFarFromOrigin",
      label: "Check geometries far from origin",
      description:
        "Flag every SceneGeometry whose local AABB centroid has " +
        "magnitude above the configured distance — typically a " +
        "georeferenced offset baked into vertex positions instead " +
        "of a mesh matrix.",
      default: false,
    },
    fields: [
      {
        ...
        label: "Max geometry centroid distance from origin",
        description:
          "Threshold for GEOMETRY_FAR_FROM_ORIGIN, in SceneModel " +
          "coordinate-system units. Shares the same numeric meaning " +
          "as objectPlacement's maxOriginDistance — the schema " +
          "duplication is intentional so each inspection's UI is " +
          "self-contained.",
        default: 1e6,
        min: 0,
        step: 1_000,
