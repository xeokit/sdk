...
import {SolidPrimitive} from "../../../base/constants";
...
export const geometryQuality: Inspection = {
  ...
  config: {
    enabled: {
      kind: "boolean",
      key: "checkGeometryQuality",
      label: "Check geometry quality",
      description:
        "Run the eight geometry-quality checks (zero-volume AABBs, " +
        "degenerate / duplicate / non-watertight / inconsistent-winding " +
        "triangles, unused / duplicate vertices, loose AABBs).",
      default: false,
    },
    fields: [
      {
        kind: "number",
        key: "minAabbFill",
        label: "Min AABB fill fraction",
        description:
          "GEOMETRY_AABB_NOT_TIGHT fires when the smallest-axis " +
          "u16 footprint in positionsCompressed covers less than this " +
          "fraction of the available [0, 65535] quantisation grid.",
        default: 0.5,
        min: 0,
        max: 1,
        step: 0.05,
  ...
  run(
    ...
    const minAabbFill = cfg.minAabbFill as number;
    ...
    for (const id in sceneModel.geometries) {
      ...
      checkGeometry(geom, sceneModel, ix, issues, minAabbFill);
    }
    return issues;
  },
};
