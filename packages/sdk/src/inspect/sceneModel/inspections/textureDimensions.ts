...
export const textureDimensions: Inspection = {
  ...
  config: {
    enabled: {
      kind: "boolean",
      key: "checkTextureSanity",
      label: "Check texture dimensions",
      description:
        "Flag textures whose width or height is not a power of " +
        "two (mipmap / wrap mode quirks) or exceeds the configured " +
        "pixel-dimension budget.",
      default: false,
    },
    fields: [
      {
        ...
        description:
          "Pixel-dimension threshold for TEXTURE_OVERSIZED. Default " +
          "4096 — the WebGL 2 minimum-guaranteed MAX_TEXTURE_SIZE; " +
          "above that, memory + upload cost grows quadratically.",
        default: 4096,
        min: 1,
        step: 256,
        unit: "px",
  ...
  run(sceneModel: SceneModel, params: InspectSceneModelParams): Issue[] {
    ...
    const maxTextureDim = cfg.maxTextureDim as number;
    const issues: Issue[] = [];
