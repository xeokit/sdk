...
/**
 * Traverses a {@link model!data.Data | Data} to collect {@link model!data.DataObject | DataObjects} that matching given search criteria.
 *
...
export class DataTraversalQuery {
  ...
  query(): void | SDKInternalException {
    ...
    if (this.startObjectId) {
      ...
      if (!startObject) {
        return new SDKInternalException(`[DataTraversalQuery.query] Cannot search DataObjects - starting DataObject not found in Data: "${this.startObjectId}"`);
      }
