 ...
 *   - Returns `ok:false` when the SceneModel is finalised, so the
 *     caller can surface the error rather than silently skipping.
  ...
  apply(issue: Issue, sceneModel: SceneModel): SDKResult<FixApplyResult> {
    ...
    if (sceneModel.finalized) {
      return {
        ok: false,
        type: SDKErrorType.InvalidOperation,
        error: `[mergeSimilarGeometries] SceneModel is finalised — instancing needs createMesh, which requires !finalized`,
      };
    }
