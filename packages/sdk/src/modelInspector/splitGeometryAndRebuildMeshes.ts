import type {SceneGeometry, SceneModel, SceneObject} from "../scene";
 ...
 * a {@link SceneGeometry} in two and fan referencing meshes onto
 ...
 *   2. Snapshot every {@link SceneMesh} that referenced the source.
 ...
 *
 * Caller is responsible for the lifecycle gate — `createGeometry`
 * / `createMesh` both require `!sceneModel.finalized`. This
 * function returns an explicit error if invoked on a finalised
 * model.
 *
...
export function splitGeometryAndRebuildMeshes(
  ...
  if (sceneModel.finalized) {
    return {
      ...
      error: `[splitGeometryAndRebuildMeshes] SceneModel is finalised — split needs createGeometry / createMesh, both of which require !finalized`,
    };
  }
