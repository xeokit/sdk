/**
 * CPU-side hidden-line elimination for the drawing projector.
 *
...
import type {HLEDepthBuffer} from "./HLEDepthBuffer";
import type {BuildHLEDepthBufferOptions} from "./BuildHLEDepthBufferOptions";
...
export async function buildHLEDepthBuffer(
  ...
  const owners: Int32Array<any> | null = withOwners ? new Int32Array(width * height) : null;
  ...
  for (const objectId of Object.keys(sourceModel.objects)) {
    ...
    for (const mesh of obj.meshes) {
      ...
      // mirror the source hierarchy in the drawing.
      let ownerIndex = -1;
