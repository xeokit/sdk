...
import type {ApplyFixesParams} from "./params/ApplyFixesParams";
...
export function applyFixes(params: ApplyFixesParams): SDKResult<ApplyFixesResult> {
  ...
  for (const issue of report.issues) {
    ...
    if (strategy.config) {
      // Layer registry-stored overrides under per-call
      // fixOverrides — per-call wins on declared keys.
      const stored = registry.getConfig(strategy);
      const merged = stored ? {...stored, ...params.fixOverrides} : params.fixOverrides;
      if (!resolveConfig(strategy.config, merged).enabled) {
        skipped.push({issue, strategy: strategy.description, reason: "disabled"});
        continue;
      }
    }
