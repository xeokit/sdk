...
import type {ModelNavigationPickFn, ModelNavigationControllerParams} from "./ModelNavigationControllerParams";
import {CameraFlightAnimation} from "../../cameraFlight";
import {CameraUpdater} from "./internal/CameraUpdater";
import {createVec2Float64, type Vec3} from "../../../base/math/vector";
import {EventDispatcher} from "strongly-typed-events";
import {KeyboardAxisViewHandler} from "./internal/KeyboardAxisViewHandler";
import {KeyboardPanRotateDollyHandler} from "./internal/KeyboardPanRotateDollyHandler";
import {MouseMiscHandler} from "./internal/MouseMiscHandler";
import {MousePanRotateDollyHandler} from "./internal/MousePanRotateDollyHandler";
import {MousePickHandler} from "./internal/MousePickHandler";
import {PanController} from "./internal/PanController";
import {PickController} from "./internal/PickController";
import {PivotController} from "./internal/PivotController";
import {TouchPanRotateAndDollyHandler} from "./internal/TouchPanRotateAndDollyHandler";
import {TouchPickHandler} from "./internal/TouchPickHandler";
import {PickResult} from "../../viewer";
import {ModelNavigationControllerEvents} from "./ModelNavigationControllerEvents";
...
export class ModelNavigationController {
  ...
  /**
   * The {@link viewing!viewer.View | View} to which this ModelNavigationController belongs.
   */
  view: View;

  /**
   * Events fired by this ModelNavigationController.
   */
  events: ModelNavigationControllerEvents;
   ...
   *
   */
  constructor(view: View, cfg: ModelNavigationControllerParams = {}) {
    ...
    // User-settable ModelNavigationController configurations
    ...
    // Current runtime state of the ModelNavigationController
    ...
    this.events = new ModelNavigationControllerEvents();
  ...
  /**
   * Sets custom mappings of keys to ````ModelNavigationController```` actions.
   *
   ...
   * which causes ````ModelNavigationController```` to use the default key mappings for that layout.
   */
  set keyMap(value: { Number: number } | number | string) {
    ...
    if (typeof value === "object") {
      ...
      switch (value) {
        ...
        case QWERTYLayout:
        case "qwerty":
          keyMap[ModelNavigationController.PAN_LEFT] = [KEY_A];
          keyMap[ModelNavigationController.PAN_RIGHT] = [KEY_D];
          keyMap[ModelNavigationController.PAN_UP] = [KEY_Z];
          keyMap[ModelNavigationController.PAN_DOWN] = [KEY_X];
          keyMap[ModelNavigationController.PAN_BACKWARDS] = [];
          keyMap[ModelNavigationController.PAN_FORWARDS] = [];
          keyMap[ModelNavigationController.DOLLY_FORWARDS] = [KEY_W, KEY_ADD];
          keyMap[ModelNavigationController.DOLLY_BACKWARDS] = [KEY_S, KEY_SUBTRACT];
          keyMap[ModelNavigationController.ROTATE_X_POS] = [KEY_DOWN_ARROW];
          keyMap[ModelNavigationController.ROTATE_X_NEG] = [KEY_UP_ARROW];
          keyMap[ModelNavigationController.ROTATE_Y_POS] = [KEY_Q, KEY_LEFT_ARROW];
          keyMap[ModelNavigationController.ROTATE_Y_NEG] = [KEY_E, KEY_RIGHT_ARROW];
          keyMap[ModelNavigationController.AXIS_VIEW_RIGHT] = [KEY_NUM_1];
          keyMap[ModelNavigationController.AXIS_VIEW_BACK] = [KEY_NUM_2];
          keyMap[ModelNavigationController.AXIS_VIEW_LEFT] = [KEY_NUM_3];
          keyMap[ModelNavigationController.AXIS_VIEW_FRONT] = [KEY_NUM_4];
          keyMap[ModelNavigationController.AXIS_VIEW_TOP] = [KEY_NUM_5];
          keyMap[ModelNavigationController.AXIS_VIEW_BOTTOM] = [KEY_NUM_6];
          break;

        case AZERTYLayout:
        case "azerty":
          keyMap[ModelNavigationController.PAN_LEFT] = [KEY_Q];
          keyMap[ModelNavigationController.PAN_RIGHT] = [KEY_D];
          keyMap[ModelNavigationController.PAN_UP] = [KEY_W];
          keyMap[ModelNavigationController.PAN_DOWN] = [KEY_X];
          keyMap[ModelNavigationController.PAN_BACKWARDS] = [];
          keyMap[ModelNavigationController.PAN_FORWARDS] = [];
          keyMap[ModelNavigationController.DOLLY_FORWARDS] = [KEY_Z, KEY_ADD];
          keyMap[ModelNavigationController.DOLLY_BACKWARDS] = [KEY_S, KEY_SUBTRACT];
          keyMap[ModelNavigationController.ROTATE_X_POS] = [KEY_DOWN_ARROW];
          keyMap[ModelNavigationController.ROTATE_X_NEG] = [KEY_UP_ARROW];
          keyMap[ModelNavigationController.ROTATE_Y_POS] = [KEY_A, KEY_LEFT_ARROW];
          keyMap[ModelNavigationController.ROTATE_Y_NEG] = [KEY_E, KEY_RIGHT_ARROW];
          keyMap[ModelNavigationController.AXIS_VIEW_RIGHT] = [KEY_NUM_1];
          keyMap[ModelNavigationController.AXIS_VIEW_BACK] = [KEY_NUM_2];
          keyMap[ModelNavigationController.AXIS_VIEW_LEFT] = [KEY_NUM_3];
  ...
  /**
   * Gets custom mappings of keys to {@link ModelNavigationController} actions.
   */
  get keyMap() {
    return this._keyMap;
  }
  ...
  /**
   * Sets the HTMl element to represent the pivot point when {@link ModelNavigationController.followPointer} is true.
   *
  ...
  /**
   *  Sets if this ````ModelNavigationController```` is active or not.
   *
   * When inactive, the ````ModelNavigationController```` will not react to input.
   *
   * Default is ````true````.
   */
  ...
  /**
   * Gets if this ````ModelNavigationController```` is active or not.
   *
   * When inactive, the ````ModelNavigationController```` will not react to input.
   *
   * Default is ````true````.
   *
   * @returns Returns ````true```` if this ````ModelNavigationController```` is active.
   */
  get active(): boolean {
   ...
   * Default is ````true````.
   *
   * Disabling mouse and touch input on ````ModelNavigationController```` is useful when we want to temporarily use mouse or
   ...
   * Default is ````true````.
   *
   * Disabling mouse and touch input on ````ModelNavigationController```` is desirable when we want to temporarily use mouse or
   ...
   *
   * Only applies when {@link ModelNavigationController.followPointer} is ````true````.
   *
   ...
   *
   * Only applies when {@link ModelNavigationController.navMode} is ````"firstPerson"````.
   *
   * Default is ````false````.
   *
  ...
  set doublePickFlyTo(value: boolean) {
    this._configs.doublePickFlyTo = value === true;
  }
   ...
   * Default is ````0.0````.
   *
   * Does not apply when {@link ModelNavigationController.navMode} is ````"planView"````, which disallows rotation.
   *
   ...
   * a panning key is depressed. See the ````ModelNavigationController```` class documentation for which keys control
   * panning.
   *
   ...
   * a rotation key is depressed. See the ````ModelNavigationController```` class documentation for which keys control
   * rotation/orbit.
   *
   ...
   *
   * ````ModelNavigationController```` makes vertical rotation half as sensitive as horizontal rotation, so that we don't tend to
  ...
  /**
   * Destroys this ````ModelNavigationController````.
   * @private
   */
  destroy() {
