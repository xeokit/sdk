    ...
    this.#updateTask = new SDKTask({
      ...
      task: () => {
        ...
        if (!configs.planView) {
          ...
          const rotateYPos = modelNavigation._isKeyDownForAction(VC.ROTATE_Y_POS, keyDownMap);
          const rotateYNeg = modelNavigation._isKeyDownForAction(VC.ROTATE_Y_NEG, keyDownMap);
          const rotateXPos = modelNavigation._isKeyDownForAction(VC.ROTATE_X_POS, keyDownMap);
          const rotateXNeg = modelNavigation._isKeyDownForAction(VC.ROTATE_X_NEG, keyDownMap);
        ...
        if (!keyDownMap[17] && !keyDownMap[18]) { // not CTRL or ALT
          ...
          const dollyBackwards = modelNavigation._isKeyDownForAction(VC.DOLLY_BACKWARDS, keyDownMap);
          const dollyForwards = modelNavigation._isKeyDownForAction(VC.DOLLY_FORWARDS, keyDownMap);
        ...
        const panForwards = modelNavigation._isKeyDownForAction(VC.PAN_FORWARDS, keyDownMap);
        const panBackwards = modelNavigation._isKeyDownForAction(VC.PAN_BACKWARDS, keyDownMap);
        const panLeft = modelNavigation._isKeyDownForAction(VC.PAN_LEFT, keyDownMap);
        const panRight = modelNavigation._isKeyDownForAction(VC.PAN_RIGHT, keyDownMap);
        const panUp = modelNavigation._isKeyDownForAction(VC.PAN_UP, keyDownMap);
        const panDown = modelNavigation._isKeyDownForAction(VC.PAN_DOWN, keyDownMap);
