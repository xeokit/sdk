...
import type {ModelNavigationController} from "../ModelNavigationController";
...
class PickController {
  ...
  constructor(modelNavigation: ModelNavigationController, configs: any) {

    this.#view = modelNavigation.view;
  ...
  update() {
    ...
    if (!this.#modelNavigation.pick) {
      console.warn("[PickController.update] No pick implementation found on ModelNavigationController.");
      this.schedulePick = false;
      this.scheduleSnapOrPick = false;
      return;
    }
    ...
    if (this.scheduleSnapOrPick || this.schedulePick) {
      ...
      const result = this.#modelNavigation.pick(this.#modelNavigation.view,{
  ...
  fireEvents() {
    ...
    const events = this.#modelNavigation.events;

    /**
     * Emit transition out of snap/surface-hover state.
     */
    if (this.hoveredSnappedOrSurfaceOff) {
      this.#modelNavigation.events.onHoverSnapOrSurfaceOff.dispatch(this.#modelNavigation, {
        canvasPos: this.pickCursorPos,
    ...
    if (this.snappedOrPicked) {
      if (this.snapPickResult) {
        ...
        events.onHoverSnapOrSurface.dispatch(this.#modelNavigation, pickResult);
        this.snapPickResult = null;
      } else {
        /**
         ...
         */
        events.onHoverSnapOrSurface.dispatch(this.#modelNavigation, this.pickResult);
      }
    } else {
      /**
       * No-op else branch.
    ...
    if (this.picked && this.pickResult && (this.pickResult.viewObject || this.pickResult.worldPos)) {
      ...
      if (this.pickResult.viewObject) {
        ...
        if (this.#lastPickedEntityId !== pickedEntityId) {
          ...
          if (this.#lastPickedEntityId !== undefined) {
            events.onHoverOut.dispatch(this.#modelNavigation, {
              viewObject: this.#view.objects[this.#lastPickedEntityId]
            });
          }

          events.onHoverEnter.dispatch(this.#modelNavigation, this.pickResult);
          this.#lastPickedEntityId = pickedEntityId;
        }
      }

      events.onHover.dispatch(this.#modelNavigation, this.pickResult);
      ...
      events.onHoverOff.dispatch(this.#modelNavigation, {
        canvasPos: this.pickCursorPos
      });
    }
