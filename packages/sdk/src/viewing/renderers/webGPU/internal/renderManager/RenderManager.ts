...
export class RenderManager {
  ...
  private _refreshSplatBatches(cache: ViewRenderCache, view: View): SDKResult<void> {
    ...
    this._rememberSplatMeshSlots(cache, splatBatchSet);
    const triangleInstances = cache.instanceFrame?.buffer
      ? (cache.batchSet?.instanceCapacity ?? 0)
      : 0;
    if (splatBatchSet.slotCount > 0 && !cache.instanceFrame?.buffer) {
      const instanceFrameResult = this._instanceBufferManager.beginFrame(
        (cache.batchSet?.projectedInstanceCapacity ?? 0) + splatBatchSet.slotCount,
        view.id
      );
      if (instanceFrameResult.ok === false) {
        return instanceFrameResult;
    ...
    cache.totalInstances = triangleInstances + splatBatchSet.slotCount;
