...
export class GPUMemoryManager implements GPUMemoryReader, GPUMemoryEditor {
   ...
   *
   * @throws {@link base!core.SDKInternalException | SDKInternalException} If called before {@link init} created the required resources.
   */
  ...
  public createBatch(options: { hasNormals?: boolean, hasUVs?: boolean, triplanar?: boolean, mipmap?: boolean } = {}): SDKResult<number> {
    ...
    const gpuMemoryBatch = new GPUMemoryBatch(index, this._renderContext, options);
   ...
   */
  public setMeshClippable(meshHandle: GPUMemoryMeshHandle, viewIndex: number, clippable: boolean) {
    const batch = this._batches[meshHandle.gpuMemoryBatchIndex];
    if (!batch) {
      throw new SDKInternalException("[GPUMemoryManager.setMeshClippable] Invalid batch index in mesh handle.");
    }
    batch.setMeshClippable(meshHandle.meshIndex, viewIndex, clippable);
  }
   ...
   * {@link model!scene.SceneMaterial | SceneMaterial} whose `linePattern` or `hatchPattern`
   ...
   * frame).
   */
  public sceneMaterialPatternChanged(material: SceneMaterial): boolean {
    let updated = false;
    for (const batch of this._batches) {
      if (batch.updateMaterialPattern(material)) updated = true;
    }
    return updated;
  }
