 ...
 */
export interface GPUMemoryConfigs {
    ...
    /**
     * Maximum number of mesh batches that can be allocated in GPU memory.
     * A mesh batch is a collection of meshes that share the same primitive type and can be rendered
     * in a single draw call.
     */
    maxMeshBatches: number;

    /**
     * Maximum number of vertices allocated in a single mesh batch.
     * A vertex may contain position, normal, color, and other attributes depending on the mesh type.
     */
    maxVerticesPerBatch: number;

    /**
     * Maximum number of indices allocated in a single mesh batch.
     * Indices are used to define the connectivity of vertices for rendering primitives.
     */
    maxIndicesPerBatch: number;

    /**
     * Maximum number of geometries that can be allocated in a single mesh batch.
     * In this context, each mesh uses exactly one geometry.
     */
    maxGeometriesPerBatch: number;

    /**
     * Maximum number of meshes that can be allocated in a single mesh batch.
     * Each mesh is associated with one geometry.
     */
    maxMeshesPerBatch: number;

    /**
     * Maximum number of primitives (triangles, lines, or points) that can be allocated in a single mesh batch.
     * A primitive can be a triangle (3 indices), a line (2 indices), or a point (no indices).
     */
    maxPrimsPerBatch: number;
}
