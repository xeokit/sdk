...
export class GPUMemoryBatch {
  ...
  /**
   * Adds a SceneMesh to data texture gpuMemoryManager.
   *
  ...
  flush(): boolean {
    ...
    didFlush = this._geometryAttribs.flush()|| didFlush;
    didFlush = this._edgeIndices.flush()|| didFlush;
    didFlush = this._positions.flush()|| didFlush;
    didFlush = this._vertexColors.flush()|| didFlush;
    didFlush = this._meshMatrices.flush()|| didFlush;
    for (let i = 0; i < 4; i++) {
