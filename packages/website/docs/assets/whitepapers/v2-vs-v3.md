...
- **V3** is a ground-up **TypeScript** rewrite organised as a **modular, bucketed
  ...
  interface with WebGL and WebGPU implementations, a separated scene/data graph,
  result-monad error handling, and symmetric **import *and* export** across far
  more formats.

If you are shipping now and your needs map onto V2's plugins, V2 is the pragmatic
choice. If you are starting fresh, need TypeScript, multi-view, WebGPU, format
export, or the newer capabilities (Gaussian splats, section caps, LOD,
validation), V3 is the forward path.
...
| Renderer                 | WebGL 2 (VBO + optional data-texture `dtxEnabled`)        | WebGL 2 and WebGPU behind a pluggable **`Renderer` interface** |
...
| Memory control            | Fixed internal allocation                                 | **Configurable GPU memory budgets** + live renderer memory stats |
...
| Conversion tooling       | Separate `xeokit-convert` package/CLI (→ XKT)             | In-SDK `conversion/` (`ModelConverter`, `xeoconvert` CLI, `ifc2gltf2xgf`), multi-format, with validation + reports |
| 2D drawings              | StoreyViews (2D plan images)                              | PDF/DWG/DXF/SVG sheet import plus generated drawing workflows in the website examples |
| Reality capture          | LAS/LAZ point clouds                                       | LAS/LAZ **+ 3D Gaussian Splatting** (`.splat`)                           |
| City-scale / tiled       | XKT whole-model streaming                                 | XGF Stream, XKT import, 3D Tiles import, and camera-driven streaming |
| Maturity                 | **Production-proven, large install base**                 | Newer architecture, broader scope, active WebGL and WebGPU backends      |
...
| --------------- | -------------- |
| `base/`         | Math (Float32 + Float64), `SDKResult`, events, IO, locale, constants |
| `model/`        | `scene/` (render graph), `data/` (semantic graph), `generation/`, `lod/` |
| `viewing/`      | `viewer/` (`Viewer`, `View`, `Camera`, `SectionPlane`, lights, effects), `renderers/` (`webGL`, `webGPU`), `navigation/`, `cameraFlight/`, `transformControls/`, `profiles/`, `adaptiveQuality/`, `lod/` |
| `formats/`      | Format-specific loaders, exporters, import options and export options |
| `spatial/`      | `collision/` (KdTree / BVH), `picking/` |
| `quality/`      | Scene + data model validation, optimization and report serialization |
| `tools/`        | `measurement/` (distance, angle) |
| `interop/`      | `bcf/` |
| `conversion/`   | Conversion pipelines + `xeoconvert` CLI |
...
to use the core (`Scene` + `Viewer` + `WebGLRenderer` or `WebGPURenderer`) without any
higher-level UI.
...
## Examples and website support code

Both generations use examples as the main proof surface, but they are organised
differently.

V2's examples are primarily plugin-oriented: each page demonstrates a `Viewer` plus one
or more plugins, usually loading an existing model asset and showing a focused UI pattern
such as navigation, metadata trees, measurements, section planes, StoreyViews or BCF.
That matches V2's product shape: application functionality is delivered as viewer plugins.

V3's website examples are broader because the SDK surface is broader. The generated
example catalog currently spans **143 examples** across:

- **Getting started** — bare model viewer, file drop, and browser-test harness pages.
- **Create** — direct `SceneModel` / `DataModel` authoring, materials, transforms,
  precision, representation sets, points, and generated scene content.
- **Import / export** — XGF, XGF Stream, XKT, IFC, glTF, OBJ, FBX, dotbim, CityJSON,
  CityGML, 3D Tiles, LAS/LAZ, USDZ, 3DXML, PDF/DWG/DXF/SVG drawings, and splats.
- **Streaming and benchmarks** — XGF chunk streams, recursive streams, 3D Tiles,
  splats, Baku-scale stress cases, procedural cityscape and renderer load tests.
- **View and interaction** — WebGL/WebGPU comparison, WebGPU picking/snapping/object
  state pages, render-path matrices, section planes, LOD, ViewLayers, effects,
  texture matrices, collision and snapping.
- **Workflows** — sun studies, heatmaps, camera tours, drawing projections, schedule
  playback, material attachment and volume overlays.

The important distinction is that these examples are not all SDK API. Some are deliberately
application-level compositions that show how to build product features on top of the
lower-level V3 buckets. This is especially true for tree views, drawings, heatmaps,
presentations, Studio panels, simulation and benchmark harnesses.

### Website code libraries

Reusable example code lives under `packages/website/libs/`, separate from
`packages/sdk/src/`, so demo glue does not accidentally become SDK contract.

| Library | Role |
| ------- | ---- |
| `libs/examples` | Example-specific runtimes such as aircraft controls, flight dynamics, physics helpers and HUDs. |
| `libs/authoring` | Helpers for examples that author `SceneModel`s, `DataModel`s, geometry, objects and properties. |
| `libs/ui` | Browser UI widgets used by examples and Studio, including context menus and tree views. |
| `libs/presentations` | Higher-level presentation/demo features: drawings, heatmaps, camera tours, schedules, sun studies, materials, section caps and volume overlays. |
| `libs/studio` | The website Studio shell: panels, dialogs, loading, picking, context menus and view management. |

The build script `packages/website/scripts/build/build-example-libs.js` bundles these
TypeScript libraries to `libs/<name>/dist/` and also supports example-local
`examples/<example-id>/src/index.ts` entrypoints. Those bundles may import SDK source
through `@xeokit/sdk` aliases during the website build, but they remain website support
code, not public SDK modules.

### Dataset generation and asset pipelines

The website also contains dataset authoring scripts. These are practical examples of the
headless V3 architecture, because they build, convert, split, validate or profile assets
without requiring a live viewer.

The boundary is intentional:

- `packages/website/scripts/` is for website maintenance: page builds, SDK docs,
  example indexes, example library bundles, screenshots, model snapshots and browser-test
  serving.
- `packages/website/tools/` is for data and performance work: model conversion, XGF stream
  generation, procedural generation, catalog indexing, LOD asset creation and renderer
  benchmarks.

The main dataset tooling groups are:

- **`tools/asset-pipeline/`** — one-off and reusable scripts for converting website model
  assets, building XGF streams, splitting streams, authoring LOD assets, generating
  3D Tiles datasets and updating model catalog metadata.
- **`tools/city-generator/`** — deterministic procedural city generation from built-in or
  OSM-derived profile JSON, including calibration, regression reports, XGF export and
  3D Tiles export paths.
- **`tools/earth-generator/`** — Natural Earth to tiled XGF Stream globe generation, with
  ...
  viewer.
- **`tools/building-generator/`** — rule-based procedural building generation from
  existing model samples, producing XGF and XGF Stream assets.
- **`tools/render-benchmarks/`** — browser-driven renderer, streaming and visual
  comparison benchmarks, including WebGPU and XGF city-scale cases.

These scripts are not substitutes for the SDK conversion API described below. They are
repo-local authoring pipelines that use the SDK to create the datasets needed by examples,
stress tests and documentation. The reusable product surface is the SDK loader/exporter,
conversion and validation API; the website tools are concrete pipeline recipes.
...
`Renderer` interface, with both `WebGLRenderer` and `WebGPURenderer` available as
backend implementations. The technique set is broader: atlas-packed PBR textures,
shader-permutation caching, fragment-stage logarithmic depth, section-plane caps,
hatch / line patterns, WebGPU packed pages and instance buffers, and
renderer-specific diagnostic surfaces.

Both support SAO, edges, x-ray, highlight, selection, and PBR materials. V3 adds bloom,
configurable tonemapping (ACES / Reinhard / linear), color grading, depth of field,
atmosphere, WebGPU FXAA post-processing, and an independent render-resolution scale
as per-`View` settings.
...
V3 renderers expose explicit GPU memory controls. `WebGLRenderer` accepts a GPU
memory budget and allocation shape:
...
`WebGPURenderer` has its own `memoryConfigs` and `getMemoryStats()` surface. It reports
packed triangle pages and segments, used bytes, instance buffer capacity, RTC tile
storage, and segment lifecycle counts. Its memory knobs include packed segment limits,
streaming build budgets, frustum/projected-size culling, and compact page options for
live or sealed streams.
...
`ModelLoader` / `ModelExporter` implementations the viewer uses, under
`conversion/`:

- **`conversion/pipeline`** — the programmatic core. A `ModelConverter` is configured
...
- **`conversion/xeoconvert`** — a CLI binary wrapping `ModelConverter`, for build scripts and
...
- **`conversion/ifc2gltf2xgf`** — the end-to-end **IFC → glTF → XGF** path. It consumes an
...
(`conversion/pipeline/reporters`): an **inspection** report (validation findings), a
...
| Section caps (filled cuts)   | —                                    | `viewing/renderers` section-plane cap rendering  |
| Distance / angle measurement | `DistanceMeasurementsPlugin`, `AngleMeasurementsPlugin` | `tools/measurement`                  |
| Annotations / markup         | `AnnotationsPlugin`                  | app-level composition                            |
| 3D zones                     | `ZonesPlugin`                        | —                                                |
| Tree view (BIM structure)    | `TreeViewPlugin`                     | data-graph driven, app builds the tree           |
| 2D storey plans              | `StoreyViewsPlugin` (plan images)    | PDF/DWG/DXF/SVG import plus generated drawing examples |
| NavCube / axis gizmo         | `NavCubePlugin`, `AxisGizmoPlugin`   | app-level composition                            |
...
| Perf: culling / fast-nav     | `ViewCullPlugin`, `FastNavPlugin`    | render bins, ViewProfiles, AdaptiveQuality, resolution scale per View |
| Picking / snap               | `Scene.pick` (vertex/edge snap)      | `spatial/picking` (ray, surface, snap-to-vertex/edge) |
| Heatmaps                     | —                                    | website workflow examples                        |
| Exploded views               | (app-level)                          | model/view transform composition                 |
| Model / data validation      | —                                    | `quality/sceneModel`, `quality/dataModel`        |
| LOD                          | —                                    | `model/lod` and `viewing/lod`                    |
...
- **WebGL and WebGPU renderer backends** — both attach through the same renderer
  contract while keeping backend-specific storage, passes, and diagnostics internal.
- **Configurable GPU memory budget + usage tracking** — renderer-specific
  `memoryConfigs` / `createMemoryConfigs` helpers to size allocation for the target
  device, plus `getMemoryUsage()` on WebGL and `getMemoryStats()` on WebGPU.
...
- **Drawing sheet import and workflows** — PDF / DWG / DXF / SVG sheets can be loaded
  into the same scene as 3D model data; the website examples include generated
  plan/elevation drawing workflows.
...
- **Representation LOD** — model-side representation sets and viewing-side LOD
  selection for switching between authored or generated representations.
...
| `DistanceMeasurementsPlugin`         | `tools/measurement`                                              |
| `TreeViewPlugin`                     | query the `Data` graph (`searchObjects`) and build the tree       |
| `StoreyViewsPlugin`                  | PDF/DWG/DXF/SVG import and app-level/generated drawing workflows |
...
- **Choose V3** when you are starting fresh, want TypeScript and explicit error
  ...
  in-app, or want the newer capabilities — WebGPU rendering, Gaussian splats,
  section caps, LOD, validation, and expanded format I/O.
