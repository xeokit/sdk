...
Architecturally, xeokit V3 emphasizes **separation of concerns**, clearly distinguishing the roles of scene data structures, viewers, and renderers within an event-driven pipeline. This modular design enables features such as multi-canvas synchronized views, extensible WebGL and WebGPU rendering implementations, and advanced inspection tooling. The renderer itself is intentionally exposed as a **transparent, inspectable system**, allowing developers to analyze performance, debug rendering behavior, and build custom diagnostic interfaces.
...
* xeokit V3 currently provides `WebGLRenderer` and `WebGPURenderer` implementations of the renderer interface.

## WebGPU Renderer

WebGPU is a first-class V3 renderer backend, not an experimental side path. It
implements the same backend-neutral renderer contract as `WebGLRenderer`: applications
still create a `Scene`, attach a `Viewer`, create one or more `View`s, and choose a
renderer implementation behind that viewer.

The WebGPU backend differs internally because it maps viewer state into WebGPU-native
resources:

* `WebGPURenderer` owns device acquisition, viewer attachment, lifecycle events, picking,
  snapping, memory diagnostics and render diagnostics.
* `ViewManager` owns per-view canvas state and coordinates the internal render graph.
* `MeshManager` translates scene meshes into renderer mesh records without making scene
  resources depend on GPU layout.
* `TriangleBatchManager` packs triangle-family geometry into large GPU pages and draw
  segments.
* `InstanceBufferManager` uploads per-object and per-view state, including transforms,
  colors, opacity, pick colors, emphasis flags and RTC tile identity.
* `RTCTileManager`, `FrameUniformManager`, `LightingManager`, `DrawOps`, `RenderManager`,
  `PickManager`, `SnapManager` and `RenderInspector` own the remaining WebGPU-specific
  render state.

This keeps the public architecture stable while letting WebGPU use a more data-oriented
backend. Scene objects are not permanently bound to WebGPU pages. Compatible meshes can
be packed together for throughput, while per-object state remains editable through
instance data.

### WebGPU rendering path

The WebGPU renderer supports the same major viewer states as the WebGL backend, including
visibility, opacity, x-ray, highlight, selection, edges, section planes, object picking
and snapping. Its draw-technique matrix currently covers triangles, points, lines and
Gaussian splats.

Triangle rendering includes:

* Depth prepass.
* PBR and flat color paths.
* Directional shadows.
* Edges and emphasis fills.
* Section clipping and section-plane caps.
* Object picking, vertex snapping and edge snapping.
* Optional logarithmic depth for large far planes.

The post-processing chain is also WebGPU-native. It renders through offscreen HDR targets
when effects require it, then composites to the canvas. Current passes include SAO,
bloom, atmosphere depth fade, depth of field, color grading, tonemapping, sRGB output
conversion and FXAA.

### WebGPU memory and streaming

WebGPU exposes explicit renderer memory controls through `memoryConfigs` and live
diagnostics through `getMemoryStats()`. The memory stats report packed triangle pages and
segments, allocated and used bytes, instance-buffer capacity, RTC tile storage and
segment lifecycle counts.

The WebGPU streaming path is designed for models that grow incrementally:

* Meshes register as chunks arrive.
* Triangle segment build work is bounded by batch-build budgets.
* Instance buffers reserve projected capacity for built and pending segments.
* Dirty instance ranges can be uploaded without rewriting the full buffer.
* Render caches can update append-only during streaming, then compact or rebuild after
  idle periods.
* `compactStreamPages` and `compactSealedStreamPages` trade append headroom for lower
  packed-page memory use in stream-heavy workloads.

`SceneModel.lifecycle` and `SceneModel.memoryPolicy` affect WebGPU segment grouping and
page headroom. `SceneModel.updateHint` remains part of the shared model contract, but the
current WebGPU renderer does not branch on it to select static versus dynamic storage.

### WebGPU diagnostics and limits

The WebGPU backend exposes a lightweight `RenderInspector`, per-view render stats and
memory stats so applications can see whether frame cost is coming from registration,
packing, uploads, batch classification, command encoding, pass execution or presentation.
Optional GPU timestamp queries are used when the adapter exposes the feature.

Current WebGPU usage remains subject to browser and adapter support. Applications should
check `WebGPURenderer.isSupported()` before requiring the backend, and should prefer the
async `WebGPURenderer.create({viewer})` path when the renderer owns adapter/device
creation. Snapshot capture is not implemented for the WebGPU backend yet.
...
* In xeokit V3, the viewer can render an independent view of the same scene to multiple canvases. In one canvas, you
  ...
  parameters, clipping planes, and other visual states for each view, including the visibilities and effects applied to the objects,
...
The website examples are an architectural validation surface for V3. They are not a
separate product layer, and they are not all SDK API. Instead, they show how the SDK's
separated `Scene`, `Data`, `Viewer`, renderer, loader, exporter, spatial and tooling
modules compose into real browser applications.

The example catalog is generated from `packages/website/examples/**/index.json` and
`index.js` files into `packages/website/examples/index.json`. It currently contains 143
published examples covering:

* Getting started flows, including a bare model viewer, file drop and browser-test page.
* Direct model authoring examples for `SceneModel`, `DataModel`, geometry, materials,
  transforms, precision, representation sets and generated scenes.
* Import and export examples for XGF, XGF Stream, XKT, IFC, glTF, OBJ, FBX, dotbim,
  CityJSON, CityGML, 3D Tiles, LAS/LAZ, USDZ, 3DXML, PDF/DWG/DXF/SVG drawings and
  Gaussian splats.
* Streaming and benchmark examples for XGF chunk streams, recursive streams, 3D Tiles,
  splats, Baku-scale stress cases, procedural cityscape and renderer load tests.
* Viewer and interaction examples for WebGL/WebGPU comparisons, WebGPU picking and
  snapping, render-path matrices, section planes, LOD, ViewLayers, effects, texture
  matrices, collision and snapping.
* Workflow examples for sun studies, heatmaps, camera tours, drawing projections,
  schedule playback, material attachment and volume overlays.

This separation matters for architecture. A low-level renderer feature such as WebGPU
picking belongs in `packages/sdk/src/viewing/renderers/webGPU` and is exercised by a
website example. A product workflow such as heatmap painting or a schedule player may
live as website support code that composes SDK primitives. The example proves the
composition, but the reusable SDK contract remains the lower-level module boundary.

## Website Code Libraries

Reusable browser/application code for examples lives under `packages/website/libs/`, not
under `packages/sdk/src/`. This keeps demo glue and higher-level presentation workflows
from hardening accidentally into public SDK API.

The project-owned website libraries are:

* `libs/examples` - example-specific runtimes such as aircraft controls, flight dynamics,
  physics helpers and HUDs.
* `libs/authoring` - helpers for examples that author `SceneModel`s, `DataModel`s,
  geometry, objects and properties.
* `libs/ui` - browser UI widgets used by examples and Studio, including context menus
  and tree views.
* `libs/presentations` - higher-level presentation/demo features such as drawings,
  heatmaps, camera tours, schedules, sun studies, material tools, section caps and
  volume overlays.
* `libs/studio` - the website Studio shell, including panels, dialogs, loading, picking,
  context menus and view management.

`packages/website/scripts/build/build-example-libs.js` bundles these TypeScript sources
to `libs/<name>/dist/`. The same build step can also bundle example-local
`examples/<example-id>/src/index.ts` entrypoints to an example's `index.js`. During that
build, website code can import SDK source through `@xeokit/sdk` aliases, which keeps the
examples close to the current implementation while preserving the distinction between SDK
modules and website-only helpers.

## Dataset Generation and Asset Pipelines

V3's headless scene/data architecture is also exercised by repository-local dataset
generation scripts. These scripts are not the SDK conversion API itself; they are
concrete pipelines that create the model assets used by examples, benchmarks and
documentation.

The repository keeps a deliberate boundary:

* `packages/website/scripts/` contains website maintenance scripts: page builds, SDK docs,
  example indexes, example library bundles, screenshots, model snapshots and browser-test
  serving.
* `packages/website/tools/` contains data and performance tooling: model conversion, XGF
  stream generation, procedural generation, catalog indexing, LOD asset creation and
  renderer benchmarks.

The main dataset-generation groups are:

* `tools/asset-pipeline/` - conversion scripts, XGF stream builders, stream splitting,
  LOD authoring utilities, 3D Tiles dataset generation and model catalog indexing.
* `tools/city-generator/` - deterministic procedural city generation from built-in or
  OSM-derived profiles, with calibration, regression reports, XGF export and 3D Tiles
  export paths.
* `tools/earth-generator/` - Natural Earth to tiled XGF Stream globe generation, with
  land, ocean, coastline, country-boundary and country-region layers plus a diagnostic
  viewer.
* `tools/building-generator/` - rule-based procedural building generation from existing
  model samples, producing XGF and XGF Stream assets.
* `tools/render-benchmarks/` - browser-driven renderer, streaming and visual comparison
  benchmarks, including WebGPU and XGF city-scale cases.

This is a practical consequence of the V3 design. The same SDK graph, loader, exporter
and validation abstractions that can run in a viewer can also run in Node.js pipelines
that author XGF streams, generate synthetic stress scenes, rebuild catalog metadata or
