...
import {LinearFilter, sRGBEncoding, TrianglesPrimitive} from "@xeokit/sdk/base/constants";
import {buildBox} from "@xeokit/sdk/model/generation/buildGeometry";
import {buildMat4} from "@xeokit/sdk/model/scene";

export async function buildArchipelago(model, yieldToHost) {
  ...
  function buildRockGeom(id, xSize, zSize, ySize) {
    const r = buildBox({ xSize, ySize, zSize });
    if (!r.ok) return;
