...
import {TreeGenerator} from "@xeokit/sdk/model/generation/treeGenerator";
...
try {
  ...
  scene = runtime.scene;
  configureLighting(runtime.view);
  bindControls();
  applyDefaults();
  rebuildForest();
  initialized = true;
  finishExample(runtime.renderer, runtime.view);
} catch (error) {
  failExample("forest-generator", error);
}
