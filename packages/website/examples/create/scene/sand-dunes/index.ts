...
import {LinearEncoding, LinearFilter, RepeatWrapping, sRGBEncoding, TrianglesPrimitive} from "@xeokit/sdk/base/constants";
import {newPixelBuffer, heightToNormal, flatMR, periodicHash2, periodicFbm, clamp01} from "@xeokit/sdk/model/generation/paintMaterials";
...
const {scene, view, renderer} = await createStandaloneRuntime({
  viewParams: {
    ...
    effects: {
      sky: {
        enabled: true,
        skyColor: [0.62, 0.74, 0.92],
        horizonColor: [0.88, 0.78, 0.58],
        groundColor: [0.62, 0.50, 0.34],
        sunEnabled: true,
        sunDirection: [0.38, 0.48, 0.79],
        sunColor: [1.0, 0.88, 0.62],
        worldUp: [0, 0, 1]
      },
      edges: {
        enabled: false
