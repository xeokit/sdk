...
studio.init().then(() => {
  ...
  const view = studio.viewManager.createView({
    ...
    // range used when rendering XGF or LAZ data.
    pointsMaterial: {
      pointSize: 2,
      roundPoints: true,
  ...
  // Create a XGFLoader to load XGF or LAZ point cloud data into the
  ...
  fetch("../../models/Nalls-Pumpkin-Hill/xgf/model.xgf").then(response => {
    ...
    response
        ...
        .then(fileData => {
          ...
          xgfLoader.load({
            ...
            // Log any errors that occur while loading the XGF or LAZ data.
            console.error(`[XGFLoader.load] Error loading XGF: ${message}`);
          });
        });
  });
});
