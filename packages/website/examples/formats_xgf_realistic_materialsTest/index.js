// Loads the bundled MaterialsTest model from XGF and exposes the
// Rendering panel — same shape and wiring as the materials-chart,
// Cityscape, Duplex and HousePlan demos. Effect parameters are not
// overridden; the SDK constructor defaults stand and the panel HTML
// mirrors them.
import * as xeokit from "../../js/xeokit-studio-bundle.js";

const XGF_URL = "../../models/MaterialsTest/xgf/model.xgf";
...
studio.init().then(async () => {
  ...
  // Identity coordinate system — the MaterialsTest XGF was authored
  // Y-up and there's no per-model coordSys remap for it; camera up is
  // [0, 1, 0] to match.
  ...
  try {
    ...
    status.textContent = `Failed to load MaterialsTest XGF: ${err.message || err}`;
    console.error(err);
  }
});
