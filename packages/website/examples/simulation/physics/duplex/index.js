...
//
// Rapier is loaded from a CDN and injected into the shared example physics
// library. xeokit never bundles Rapier itself, so we pull the WASM-bundled
// compat build (`@dimforge/rapier3d-compat`) at runtime and pass the
// namespace straight in.
//
...
studio.init().then(() => {
  ...
  Promise.all([xgfLoaded, rapierReady]).then(([, RAPIER]) => {
    ...
    const slabBoxGeom = mustOK(xeokit.model.generation.buildGeometry.buildBox({
    ...
    console.error("simulation/physics/duplex:", err);
  });
});
