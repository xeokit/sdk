...
import {XGFStreamingLoader, readXGFStreamingIndex} from "@xeokit/sdk/formats/xgfstream";
...
import {WEBGPU_RENDER_CONFIG_PROFILES, WebGPURenderer} from "@xeokit/sdk/viewing/renderers/webGPU";
import {Viewer} from "@xeokit/sdk/viewing/viewer";
import {signalExampleLoadedOnNextRender} from "../../../utils/snapshotReady.js";
import {getRequestedRenderer} from "../../../utils/standaloneRuntime.js";
...
const HOVER_PICK_SETTLE_MS = 140;
const HOVER_PICK_MIN_INTERVAL_MS = 220;
...
async function loadCountrySemantics(ui) {
  ...
  try {
    ...
    const importer = new DataModelImporter();
    await importer.load({
...
function installCountryHover(picker, view, countrySemantics, ui, options = {}) {
  renderCountrySemantic(ui, null);
  if (!countrySemantics) {
    return;
  }
  ...
  const countryObjectIds = colorize ? createCountryObjectIdCache(view) : null;
  ...
  canvas.addEventListener("pointermove", (event) => {
    ...
    pendingEvent = event;
    clearHoverTimer();
    const elapsed = performance.now() - lastHoverPickTime;
    hoverTimer = window.setTimeout(() => {
      hoverTimer = 0;
      pickCountryUnderPointer();
    }, Math.max(HOVER_PICK_SETTLE_MS, HOVER_PICK_MIN_INTERVAL_MS - elapsed));
  });

  canvas.addEventListener("pointerleave", () => {
    clearHoverTimer();
    if (colorize) {
      colorizeHoveredCountry(view, countryObjectIds, lastCountryKey, null);
    }
    lastCountryKey = null;
    pendingEvent = null;
    renderCountrySemantic(ui, null);
  ...
  function pickCountryUnderPointer() {
    if (!pendingEvent || dragging || pendingEvent.buttons !== 0) {
      return;
    }
    ...
    if (colorize) {
      colorizeHoveredCountry(view, countryObjectIds, lastCountryKey, countryKey);
    }
    lastCountryKey = countryKey;
    const dataObject = countryKey?.dataObjectId
...
function createCountryObjectIdCache(view) {
  const countryObjectIds = new Map();
  for (const objectId of Object.keys(view.objects || {})) {
    const match = /^earth\.countryRegion\.country\.([^.]+)\./.exec(objectId);
    if (!match) {
      continue;
    }
    let objectIds = countryObjectIds.get(match[1]);
    if (!objectIds) {
      objectIds = [];
      countryObjectIds.set(match[1], objectIds);
    }
    objectIds.push(objectId);
  }
  return countryObjectIds;
}

function colorizeHoveredCountry(view, countryObjectIds, previousCountryKey, nextCountryKey) {
  if (previousCountryKey) {
    const previousObjectIds = countryObjectIds.get(previousCountryKey.countryId) || [];
    if (previousObjectIds.length > 0) {
      view.setObjectsColorized(previousObjectIds, null);
    }
  }
  if (nextCountryKey) {
    const nextObjectIds = countryObjectIds.get(nextCountryKey.countryId) || [];
...
async function createRenderer(viewer) {
  const rendererName = getRequestedRenderer();
  ...
  try {
    const result = await WebGPURenderer.create({
      viewer,
      renderConfigs: WEBGPU_RENDER_CONFIG_PROFILES.largeModel
    });
    if (!result.ok) {
...
function restyleSurfaceFillMeshes(sceneModel) {
  const styled = sceneModel.__earthStyledSurfaceMeshes || (sceneModel.__earthStyledSurfaceMeshes = new Set());
  for (const mesh of Object.values(sceneModel.meshes || {})) {
    if (!mesh || styled.has(mesh.id)) {
      continue;
    }
...
function restyleCoastlineMeshes(sceneModel) {
  const styled = sceneModel.__earthStyledCoastlineMeshes || (sceneModel.__earthStyledCoastlineMeshes = new Set());
  for (const mesh of Object.values(sceneModel.meshes || {})) {
    if (!mesh || styled.has(mesh.id) || mesh.materialId !== "earth.coastline") {
      continue;
    }
...
function restyleCountryBoundaryMeshes(sceneModel) {
  const styled = sceneModel.__earthStyledCountryBoundaryMeshes || (sceneModel.__earthStyledCountryBoundaryMeshes = new Set());
  for (const mesh of Object.values(sceneModel.meshes || {})) {
    if (!mesh || styled.has(mesh.id) || mesh.materialId !== "earth.countryBoundary") {
      continue;
    }
...
function createCountryLabel(model, options) {
  const textGeometry = must(buildVectorText({
