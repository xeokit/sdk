...
async function main() {
  ...
  // Create the SDK runtime explicitly; helper functions only provide the shared
  // streaming defaults used by the sibling examples.
  ...
  try {
    ...
    const sceneModel = must(scene.createModel({
      id: "Lyon",
      updateHint: "static",
      // Lyon is georeferenced by the exported runtime index; applying that
      // coordinate system keeps frustum streaming and navigation in sync.
    ...
    render(ui, streamController);
    bindCameraStreaming(viewer, view, streamController, ui.stallStreamingToggle);
