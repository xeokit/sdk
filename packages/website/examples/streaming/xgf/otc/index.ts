...
async function main() {
  // Keep SDK setup visible in the example: the shared utilities only hide the
  // repeated streaming profile values, not the Scene/Viewer/View ownership.
  ...
  try {
    ...
    const sceneModel = must(scene.createModel({
      ...
      updateHint: "dynamic",
      // Follow the runtime index so XGF geometry and the DataModel share the
    ...
    await new DataModelImporter().load({
      fileData: await fetchJSON(`${MODEL_BASE}/datamodel/model.json`),
      dataModel
    });
    ...
    window.otcInputController = inputController;
    ...
    bindIssueCards(view, streamController, cameraFlight);
  } catch (error) {
    console.error(error);
  }
}
...
function bindIssueCards(view, streamController, cameraFlight) {
