...
async function main() {
  ...
  // Create each SDK component here so the recursive streaming example remains
  // standalone while sharing only repetitive runtime defaults.
  ...
  const view = must(viewer.createView(createFastStreamingViewParams({
    ...
    viewId: "demoView",
    camera: {
      perspectiveProjection: {fov: INITIAL_VIEWPOINT.fov, near: NEAR_CLIP, far: FAR_CLIP},
  ...
  try {
    ...
    const scheduleRender = () => {
      ...
      window.requestAnimationFrame(() => {
        ...
        updateRenderDiagnostics(ui, renderer);
    ...
    window.recursiveStreamingExample = {
      ...
      streamController,
      viewCulling,
      sampleRenderPath: () => updateRenderDiagnostics(ui, renderer)
    };
    updateRenderDiagnostics(ui, renderer);
    viewCulling.requestCull();
    renderer.events.onViewRendered.subscribe((_, renderedView) => {
...
function updateRenderDiagnostics(ui, renderer) {
  const memory = getGeometryStorageSnapshot(renderer);
  if (ui.geometryStorageStatus) {
    ui.geometryStorageStatus.textContent = `DTX ${memory.dtxBatches}, VBO ${memory.vboBatches}`;
  }

  const frameStats = getLastFrameVBOStats(renderer);
...
function getGeometryStorageSnapshot(renderer) {
  ...
  const getMemoryInspector = renderer?.getMemoryInspector;
  ...
  const result = getMemoryInspector.call(renderer);
...
function getLastFrameVBOStats(renderer) {
  if (renderer?.rendering === false) {
    return null;
  }
  const getRenderInspector = renderer?.getRenderInspector;
  if (typeof getRenderInspector !== "function") {
    return null;
  }
  const result = getRenderInspector.call(renderer);
  if (!result.ok) {
    return null;
  }
...
function applyViewpointProjection(view, viewpoint) {
  ...
  view.camera.perspectiveProjection.near = NEAR_CLIP;
