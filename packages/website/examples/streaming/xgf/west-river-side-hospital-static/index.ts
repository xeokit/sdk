...
async function main() {
  ...
  // Explicit SDK setup mirrors the import examples while the helpers keep the
  // common WebGPU streaming profile in one place.
  ...
  try {
    ...
    const sceneModel = must(scene.createModel({
      id: MODEL_ID,
      updateHint: "static",
      // The stream index carries the exported coordinate system for the
      // hospital chunks, so no hard-coded axis conversion is needed here.
