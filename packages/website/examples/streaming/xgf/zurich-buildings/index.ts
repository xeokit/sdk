...
const VIEWPOINT_MOTION_STORAGE_KEY = "xeokit.streaming/xgf/zurich-buildings.viewpointMotion";
...
async function main() {
  const ui = {
    ...
    signalReady: createReadyHandler(),
  ...
  try {
    ...
    // Build the SDK runtime explicitly, matching the standalone import examples.
    ...
    const sceneModel = must(scene.createModel({
      ...
      updateHint: "static",
      // Buildings and terrain are separate XGF streams; each keeps its own
      // exported coordinate system from the matching runtime index.
    ...
    terrainStreamController = new XGFViewStreamController({
      index: terrainIndex,
      loader: new XGFStreamingLoader(),
    ...
    const cameraStreaming = bindCameraStreaming(viewer, streamGroup, ui.stallStreamingToggle);
...
function bindCameraStreaming(viewer, streamController, stallStreamingToggle) {
  let resumeTimer;
