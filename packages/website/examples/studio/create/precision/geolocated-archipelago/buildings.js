...
export async function placeBuildings(xeokit, studio, model, {
  ...
  if (duplexAnchor) {
    ...
    const pedGeom = xeokit.model.generation.buildGeometry.buildBox({
  ...
  if ( houseAnchor) {
    ...
    const slabGeom = xeokit.model.generation.buildGeometry.buildBox({
    ...
    const houseDataResult  = studio.data.createModel({ id: "house" });
    ...
    if (houseSceneResult.ok && houseDataResult.ok) {
      try {
        await studio.loadModel({
          id:         "house",
          src:        "../../../../../models/IfcOpenHouse4/ifc/model.ifc",
          format:     "ifc",
  ...
  if (ferryResult.ok) {
    try {
      await studio.loadModel({
        id:     "ferry",
        src:    "../../../../../models/Ferry/gltf/model.glb",
        format: "gltf",
