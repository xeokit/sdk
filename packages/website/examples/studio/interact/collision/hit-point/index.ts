...
import {failExample, finishExample, mustElement, mustOk, toNavigationPick, configureExampleRenderer} from "../../../../utils/standaloneRuntime.js";
...
main().catch((error) => failExample("studio/interact/collision/hit-point", error));

async function main() {
  // Build the SDK runtime explicitly. The city is generated into a SceneModel,
  // then queried with SceneRaycaster; renderer picking is deliberately spied on
  // below to prove this interaction path stays CPU-side.
  ...
  buildAndRegister("box",
    () => buildBox({ xSize: 1, ySize: 1, zSize: 1 }));

  buildAndRegister("cyl", () => buildCylinder({
    radiusTop: 1, radiusBottom: 1, height: 1,
    radialSegments: 28, heightSegments: 1
  }));

  buildAndRegister("tcyl", () => buildCylinder({
    radiusTop: 0.45, radiusBottom: 1, height: 1,
    radialSegments: 28, heightSegments: 1
  }));

  buildAndRegister("sph", () => buildSphere({
  ...
  const view = mustOk(viewer.createView({
    id: "cityHitPointCollisionView",
    htmlElement: mustElement("demoCanvas"),
    backgroundColor: [0.93, 0.95, 0.98],
    camera: {
      eye:  [halfCity * 0.92, -halfCity * 1.35, halfCity * 0.62],
      look: [0, 0, 10],
      up:   [0, 0, 1]
    },
    effects: {
      edges: {enabled: false},
      sao: {enabled: true, intensity: 0.06, scale: 0.75},
  ...
  // accidental GPU-pick paths are visible without crashing.
  //
  // If the counter ever leaves zero during interaction, it means something on
  // the demo path is still going through the framebuffer pick.
  // ---------------------------------------------------------------------
  let gpuPickCalls = 0;
  ...
  const inputController = new ModelNavigationController(view, {
    // Navigation uses the same CPU raycaster as the HUD, keeping pointer-follow
    // dolly/pivot behavior while preserving the example's zero-GPU-pick contract.
    pick: (_view, pickParams) => {
      const result = picker.pick({view, canvasPos: pickParams.canvasPos});
      return {ok: true, value: result.ok && result.value.hit ? toNavigationPick(view, result.value, pickParams.canvasPos) : null};
    },
    followPointer: true,
    rotationInertia: 0,
    panInertia: 0,
    ...
    keyboardDollyRate: 40,
    keyboardPanRate: 18,
    mouseWheelDollyRate: 260,
    touchDollyRate: 0.24
  });
  ...
  window.cityHitPointCollisionExample = {scene, viewer, view, renderer, picker, inputController, sceneModel};
}
