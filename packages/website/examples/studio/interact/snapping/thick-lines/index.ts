...
import {addRadioGroup, failExample, finishExample, mustElement, mustOk} from "../../../../utils/standaloneRuntime.js";

const SNAP_RADIUS_PX = 30;
const LINE_WIDTH_PX  = 6;
const THICK_LINES_COORDINATE_SYSTEM = {
  basis: [
    1, 0, 0,
    0, 1, 0,
    0, 0, 1
...
main().catch((error) => failExample("studio/interact/snapping/thick-lines", error));

async function main() {
  // Thick-line snapping is a WebGL renderer feature. Construct the runtime
  // explicitly so the renderer, line material, and pick flags are all visible.
  const scene = new Scene();
  const viewer = new Viewer({scene});
  const view = mustOk(viewer.createView({
    id: "thickLineSnapView",
  ...
  const sceneModel = mustOk(scene.createModel({
    id: "thickLines",
    coordinateSystem: THICK_LINES_COORDINATE_SYSTEM,
    updateHint: "static"
  }));
  ...
  const boxLines  = mustOk(buildBoxLines({xSize: 1.6, ySize: 1.6, zSize: 1.6}));
  ...
  mustOk(sceneModel.fromParams({
    ...
    geometries: [
      {id: "box",       primitive: LinesPrimitive, positions: boxLines.positions,  indices: boxLines.indices},
      {id: "pentagram", primitive: LinesPrimitive, positions: pentagram.positions, indices: pentagram.indices},
      {id: "zpoly",     primitive: LinesPrimitive, positions: zPoly.positions,     indices: zPoly.indices},
      {id: "triangle",  primitive: LinesPrimitive, positions: triangle.positions,  indices: triangle.indices},
      {id: "plus",      primitive: LinesPrimitive, positions: plusSign.positions,  indices: plusSign.indices},
      {id: "octagon",   primitive: LinesPrimitive, positions: octagon.positions,   indices: octagon.indices},
    ],
  ...
  view.htmlElement.addEventListener("mousemove", (e) => {
    ...
    markerEl.style.left = `${rect.left + snappedCanvasPos[0]}px`;
    markerEl.style.top  = `${rect.top + snappedCanvasPos[1]}px`;
  ...
  controls.id = "linePatternControls";
  controls.style.cssText = "position:absolute;left:12px;bottom:12px;z-index:300000;background:rgba(255,255,255,0.94);border:1px solid rgba(0,0,0,0.12);border-radius:8px;padding:8px 10px;font:12px system-ui,sans-serif;";
  document.body.appendChild(controls);
  addRadioGroup(controls, "Box line pattern", view.linesMaterial.linePattern, [
    {value: "solid",      label: "solid"},
    {value: "dashed",     label: "dashed"},
    {value: "dotted",     label: "dotted"},
    {value: "dashDot",    label: "dash-dot"},
    {value: "dashDotDot", label: "dash-dot-dot"}
  ], (v) => { view.linesMaterial.linePattern = v; });

  finishExample(renderer, view);
  window.thickLinesSnappingExample = {scene, viewer, view, renderer, sceneModel};
}
