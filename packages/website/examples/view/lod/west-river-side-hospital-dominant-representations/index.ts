...
const WEST_RIVER_SIDE_HOSPITAL_COORDINATE_SYSTEM = {
  basis: [
    1, 0, 0,
    0, 1, 0,
    0, 0, 1
...
const statusElement = mustElement("status");
...
async function main() {
  // The hospital is authored in meters with the same basis as xeokit's default
  // Z-up Scene. Passing this explicitly to SceneModel documents the model-space
  // contract that XGFLoader uses when it creates geometry and object transforms.
  const modelCoordinateSystem = WEST_RIVER_SIDE_HOSPITAL_COORDINATE_SYSTEM;
