...
function createGeometries(sceneModel) {
  ...
  const cylinder = mustBuild(xeokit.model.generation.buildGeometry.buildCylinder({
    radiusTop: 1,
    radiusBottom: 1,
    height: 1,
    radialSegments: 36,
  ...
  const taperedCylinder = mustBuild(xeokit.model.generation.buildGeometry.buildCylinder({
    radiusTop: 1.15,
    radiusBottom: 0.78,
    height: 1,
    radialSegments: 36,
...
function createTextures(sceneModel) {
  const {LinearEncoding, LinearFilter, LinearMipmapLinearFilter, RepeatWrapping, sRGBEncoding} = xeokit.base.constants;
