...
async function main() {
  ...
  const {getSceneCollisionIndex} = xeokit.spatial.collision;
...
function ensureIBL(view, runtime) {
  ...
  const hdrPixels = paint.paintSunSkyHDR(512, 256, {sunDirection: SUN_DIRECTION});
  const hdrBuffer = paint.encodeRadianceHDR(hdrPixels, 512, 256);
...
function createMaterialComparisonStrip(sceneModel) {
  ...
  const probeBox = mustBuild(xeokit.model.generation.buildGeometry.buildBox({
    xSize: 1,
    ySize: 1,
    zSize: 1
  }));
