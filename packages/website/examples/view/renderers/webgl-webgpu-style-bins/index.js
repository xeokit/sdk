...
function applyToolToObject(objectId) {
  ...
  for (const view of [runtime.webglView, runtime.webgpuView].filter(Boolean)) {
    ...
    for (const binId of STYLE_BIN_IDS) {
      mustCreate(viewObject.setStyleBin(binId, false));
    }
    if (runtime.activeTool !== "clear") {
      mustCreate(viewObject.setStyleBin(runtime.activeTool, true));
    }
    view.needsRender();
  }
  updateLegend();
...
function clearBins(view, objectIds) {
  for (const binId of STYLE_BIN_IDS) {
    mustCreate(view.setObjectsInStyleBin(binId, objectIds, false));
  }
}
...
function mustCreate(result) {
  if (!result || result.ok === false) {
    throw new Error(result?.error || "SDK operation failed");
  }
  return result.value;
}
