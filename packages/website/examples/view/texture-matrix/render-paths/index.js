...
studio.init().then(() => {
  ...
  const brick = xeokit.model.generation.paintMaterials.paintBrick(256);
  ...
  for (let row = 0; row < 4; row++) {
    for (let col = 0; col < 4; col++) {
      ...
      // `generation/buildGeometry/buildVectorText`. Most caps and
      ...
      const labelGeom = mustBuild(xeokit.model.generation.buildGeometry.buildVectorText({
        origin: [
          x - halfWidth,
