import {LinearEncoding, LinearFilter, LinearMipMapLinearFilter, LinesPrimitive, PointsPrimitive, TrianglesPrimitive, sRGBEncoding} from "@xeokit/sdk/base/constants";
import {buildBox, buildBoxLines, buildSphere} from "@xeokit/sdk/model/generation/buildGeometry";
...
import {mustOk} from "../../../utils/standaloneRuntime.js";
...
async function rebuildRenderer(cfg) {
  ...
  if (cfg.renderer === "webgpu") {
    ...
    nextRenderer = mustOk(await runtime.WebGPURenderer.create({
...
function rebuildSceneModel(cfg) {
  ...
  const sceneModel = mustOk(runtime.scene.createModel({
  ...
  mustOk(sceneModel.beginBatch({id: "renderPathMatrixBatch"}));
  try {
...
function createDiagnosticTextures(sceneModel, cfg) {
  ...
  const dataMinFilter = LinearFilter;
...
function createDiagnosticGeometry(sceneModel, cfg) {
  ...
  if (cfg.primitive === "lines") {
    const lines = mustBuild(buildBoxLines({
      xSize: 1.7,
      ySize: 1.7,
      zSize: 1.7
    }));
...
function createTriangleGeometry(sceneModel, cfg) {
  ...
  mustOk(sceneModel.createGeometry(params));
}
...
function createDiagnosticMesh(sceneModel, cfg, position, scale) {
  ...
  mustOk(sceneModel.createMesh(meshParams));
...
function configureView(view, cfg) {
  ...
  if (cfg.ibl && !runtime.iblEnvironmentReady) {
    ...
    const hdrPixels = paintSunSkyHDR(512, 256, {sunDirection});
