...
async function main() {
  ...
  if (!renderer.rendering) {
    ...
    if (!renderer.rendering) {
      reportError("[view/webgpu/snap-edge] WebGPU renderer is attached to the Viewer, but has no active ViewManager.");
      return;
    }
  }
...
function renderReady() {
  ...
  status.innerHTML =
    "<strong>WebGPU Edge Snap</strong>" +
    "<span>Move near a table corner or silhouette edge. Uses async GPU snap readback when available, with sync CPU fallback.</span>";
}
...
function renderSnapResult(pickResult, path, durationMs) {
  if (!pickResult) {
    ...
    status.innerHTML =
      "<strong>WebGPU Edge Snap</strong>" +
      `<span>${escapeHTML(path)}: no pickable object under the cursor. <code>${durationMs.toFixed(2)}ms</code></span>`;
    return;
  }

  if (!pickResult.snappedToEdge || !pickResult.snappedCanvasPos) {
    marker.style.display = "none";
    status.innerHTML =
      "<strong>WebGPU Edge Snap</strong>" +
      `<span>Path: <code>${escapeHTML(path)}</code> <code>${durationMs.toFixed(2)}ms</code></span><br>` +
      `<span>Surface object: <code>${escapeHTML(pickResult.viewObject?.id || pickResult.sceneObject?.id || "none")}</code></span><br>` +
      "<span>No edge within the snap radius.</span>";
    return;
  }
