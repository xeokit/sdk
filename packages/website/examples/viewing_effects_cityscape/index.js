// Full effects-stack tour on the Cityscape XGF model.
//
// The Rendering panel is the same shape as the materials-chart demo:
// pick a render-mode preset and tune individual effects. Effect
// settings are left at the SDK constructor defaults — the panel's
// initial slider/select states (in index.html) mirror those defaults.
...
studio.init().then(() => {
  ...
  // SceneModel uses the same Z-up basis as the Cityscape XGF was authored
  // with, so the model lands the right way up under camera up = [0,0,1].
  ...
  // Initial camera is just a placeholder pointing at the origin; we
  // re-frame to the real model bounds after the XGF loads via viewFit.
  ...
  fetch("../../models/Cityscape/xgf/model.xgf")
    ...
    .catch(err => {
      // Log the full Error (with stack) so we can see whether the failure
      // came from the loader, viewFit, or wireUpPanel.
      console.error("Cityscape example failed:", err);
    });
});
