...
import {createModelNavigationPick} from "../../../utils/workflowRuntime.js";
...
async function main() {
  ...
  const renderer = await createExampleRenderer(viewer, {grid: false, sky: false});
  ...
  const fileData = await fetchArrayBuffer("../../../../models/Duplex/ifc/model.ifc");
  const loadResult = await ifcLoader.load({fileData, sceneModel: sourceSceneModel, dataModel});
  if (loadResult && loadResult.ok === false) {
    throw new Error(loadResult.error);
        ...
        // from the shared collision index so the three planes
        // sit on the model's true world-space extent.
        const aabb = getSceneCollisionIndex(scene).getSceneAABB();
        ...
        // an empty target.
        if (!presentations.drawings.canBuildDrawing(
        ...
        const buildProjections = async (cfg) => {
          ...
          for (const targetModel of targets.values()) {
            presentations.drawings.clearDrawing(targetModel);
          }
          targets.clear();
          ...
          // toggle is off).
          if (capsModel) {
            presentations.sectionCaps.clearSectionCaps(capsModel);
            capsModel = null;
          }
          ...
          for (const {face, isPlan} of faces) {
            ...
            const projRes = await presentations.drawings.buildDrawing({
        ...
        const composeSectionCaps = async (cfg, planTarget, myGen) => {
          ...
          const capsBuild = await presentations.sectionCaps.buildSectionCaps({
            sourceModel: sourceSceneModel,
          ...
          // renders in undefined mode (set on the View above);
          ...
          const capsProj = await presentations.drawings.buildDrawing({
...
main().catch((error) => failExample("workflows/drawings/duplex", error));
