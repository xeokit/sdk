...
import {buildGeometry, paintMaterials} from "@xeokit/sdk/model/generation";
...
async function main() {
    ...
    const sceneModel = mustOk(scene.createModel({
        id: "demoPipes",
        // Procedural pipework is authored directly in metre-based model
        // coordinates. Keep the SceneModel identity-based so the moving
        // heat source and cached world-space distances use the same frame.
    ...
    // SceneObject. The `geom` field is the result of a generation build;
    ...
    parts.push({
        id: "archPipe",
        position: [6, 0, 0],
        geom: buildGeometry.buildExtrude({
    ...
    // ── Wire the generation output into SceneGeometries / Meshes.
    // Per-geometry cache of the generation-provided local-space normals.
    ...
    // generation builders both emit correctly-oriented outward normals,
    ...
    // materials / UVs. preserveExistingUvs honours the generation UVs;
    ...
    const sphereResult = buildGeometry.buildSphere({
        radius: 0.4,
        widthSegments: 24,
        heightSegments: 18
    });
    ...
    // generation builders DO emit `normals`, but we don't pass those into
    ...
    function tick(now) {
        if (now - tLastUpdate >= UPDATE_INTERVAL_MS) {
            ...
            for (let t = 0; t < liveTargets.length; t++) {
                ...
                paintMaterials.repaintHeatMapColor({
...
main().catch((error) => failExample("workflows/heatmaps/solar-pipes", error));
