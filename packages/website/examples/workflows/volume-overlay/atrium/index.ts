...
import {VolumeOverlayPanel} from "../../../../libs/studio/src/panels/volumeOverlayPanel/VolumeOverlayPanel.ts";
...
async function main() {
  ...
  const vol = presentations.volumeOverlay;
  ...
  const sceneModel = mustOk(scene.createModel({
    id: "atrium",
    // Procedural geometry is authored directly in metre coordinates.
    // Keep the model basis Z-up so the voxel field bounds and
    // streamlines align with the visible atrium geometry.
  ...
  function addBox(cx, cy, cz, sx, sy, sz, matId) {
    ...
    mustOk(sceneModel.createObject({ id, meshIds: [`${id}_mesh`] }));
  }
...
main().catch((error) => failExample("workflows/volume-overlay/atrium", error));
