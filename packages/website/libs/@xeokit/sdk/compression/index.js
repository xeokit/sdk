 ...
 * @module compression
 */
import { createMat3, createMat4, createVec3, identityMat3, identityMat4, mulMat3, mulMat4, normalizeVec3, scalingMat3v, scalingMat4v, transformVec3, translationMat3v, translationMat4v } from "../matrix";
 ...
 * @param max
 */
export function getPositions3MinMax(array, min, max) {
    let i, j;
 ...
 */
export function createPositions3DecompressMat4(aabb, positionsDecompressMatrix) {
 ...
 */
export function compressPositions3(array, min, max) {
 ...
 * @param dest
 */
export function compressPoint3WithAABB3(p, aabb, dest = p) {
 ...
 * @param dest
 */
export function decompressPoint3WithMat4(position, decompressMatrix, dest = position) {
 ...
 * @param dest
 */
export function decompressPoint3WithAABB3(position, aabb, dest = position) {
 ...
 * @param dest
 */
export function decompressAABB3WithMat4(aabb, decompressMatrix, dest = aabb) {
 ...
 * @param dest
 */
export function decompressAABB3WithAABB3(aabb, aabb2, dest = aabb) {
 ...
 * @param dest
 */
export function decompressPositions3WithMat4(positions, decompressMatrix, dest = new Float32Array(positions.length)) {
 ...
 * @param dest
 */
export function decompressPositions3WithAABB3(positions, aabb, dest = new Float32Array(positions.length)) {
    const xScale = (aabb[3] - aabb[0]) / 65535;
    const xOffset = aabb[0];
    const yScale = (aabb[4] - aabb[1]) / 65535;
    const yOffset = aabb[1];
 ...
 * @param array
 */
export function getUVBounds(array) {
 ...
 * @param array
 */
export function compressNormals(array) {
...
/**
 */
export function decompressUV(uv, decompressMatrix, dest = new Float32Array(2)) {
 ...
 *
 */
export function decompressUVs(uvs, decompressMatrix, dest = new Float32Array(uvs.length)) {
 ...
 *
 */
export function decompressNormal(oct, result) {
    let x = oct[0];
    let y = oct[1];
    x = (2 * x + 1) / 255;
    y = (2 * y + 1) / 255;
 ...
 *
 */
export function decompressNormals(octs, result) {
 ...
 *
 */
function octDecodeVec2s(octs, result) {
    for (let i = 0, j = 0, len = octs.length; i < len; i += 2) {
        let x = octs[i + 0];
        let y = octs[i + 1];
        x = (2 * x + 1) / 255;
 ...
 * @private
 */
export function quantizePositions3AndCreateMat4(positions, aabb, positionsDecompressMatrix) {
 ...
 * @private
 */
export function quantizePositions3(positions, aabb) {
    const lenPositions = positions.length;
    const positionsCompressed = new Uint16Array(lenPositions);
    const xmin = aabb[0];
    const ymin = aabb[1];
 ...
 * @private
 */
export function transformAndOctEncodeNormals(worldNormalMatrix, normals, lenNormals, compressedNormals, lenCompressedNormals) {
 ...
 * @private
 */
export function octEncodeNormals(normals) {
 ...
 * @private
 */
export function octEncodeVec3(p, xfunc, yfunc) {
 ...
 * @private
 */
export function octEncodeNormal(array, i, xfunc, yfunc) {
    let x = array[i] / (Math.abs(array[i]) + Math.abs(array[i + 1]) + Math.abs(array[i + 2]));
    let y = array[i + 1] / (Math.abs(array[i]) + Math.abs(array[i + 1]) + Math.abs(array[i + 2]));
    if (array[i + 2] < 0) {
        let tempx = (1 - Math.abs(y)) * (x >= 0 ? 1 : -1);
 ...
 */
export function compressRGBColors(colors) {
