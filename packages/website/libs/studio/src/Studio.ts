import {Scene, SceneModel, type SceneModelStats, type CoordinateSystemParams, type SceneModelUpdateHint} from "@xeokit/sdk/model/scene";
import {Data, DataModel, type DataModelStats} from "@xeokit/sdk/model/data";
import {AmbientLight, DirLight, View, Viewer, ViewObject, type ViewParams} from "@xeokit/sdk/viewing/viewer";
import {type MemoryConfigs, type MemoryUsage, WebGLRenderer} from "@xeokit/sdk/viewing/renderers/webGL";
import {WebGPURenderer, type WebGPURendererParams} from "@xeokit/sdk/viewing/renderers/webGPU";
import type {RenderStats} from "@xeokit/sdk/viewing/renderers/webGL/internal/inspectors";
import type {Renderer} from "@xeokit/sdk/viewing/rendering";
import {EventsLogger, getGlobalTaskRunner, sdkProgress, SDKErrorType, type SDKResult, SDKTask} from "@xeokit/sdk/base/core";
...
import type {TransformControlsMode, TransformControlsTarget} from "@xeokit/sdk/viewing/transformControls";
...
const STUDIO_PROFILE_SUN_ID = "__xeokit_studio_profile_sun";
const STUDIO_PROFILE_AMBIENT_ID = "__xeokit_studio_profile_ambient";
...
export class Studio {
   ...
   * `views` map (records of `view`, `cameraFlight`, `modelNavigation`),
   ...
   * ModelNavigationController adapter: `studio.picking.pickForModelNavigationController(view, params)`.
   */
  ...
  public async init(cfg: StudioConfig = {}): Promise<any> {
    ...
    this.viewManager = new ViewManager({
        ...
        pickFn: (view, pickParams) => this.picking.pickForModelNavigationController(view, pickParams),
      },
      {
   ...
   * its ModelNavigationController — layers on the right-click context menus
  ...
  private _onViewCreated(view: View, record: ViewRecord, params: StudioCreateViewParams): void {
    ...
    // browser menu suppression is the {@link ModelNavigationController}'s job
    // (it sets `view.htmlElement.oncontextmenu = e => e.preventDefault()`
    // during construction). Examples that don't instantiate a
    // ModelNavigationController will see the browser's native menu fire alongside
    ...
    this._configureProfileLighting(view);
  ...
  private _configureProfileLighting(view: View): void {
    view.clearLights();
    new AmbientLight(view, {
      id: STUDIO_PROFILE_AMBIENT_ID,
      color: [1.0, 1.0, 1.0],
      intensity: 0.08
    });
    new DirLight(view, {
      id: STUDIO_PROFILE_SUN_ID,
      dir: Array.from(view.effects.shadows.direction) as [number, number, number],
      color: [1.0, 0.96, 0.86],
      intensity: 1.45,
      space: "world"
    });
