 ...
 * per enabled {@link viewing!profiles.ViewProfiles | ViewProfiles}:
 ...
 *
 * ```ts
 * import {AdaptiveQualityPanel} from "@xeokit/website-studio";
 *
...
export class AdaptiveQualityPanel extends FloatingPanelBase {
  ...
  private _applyConfigToActiveAdapters(): void {
    for (const view of this.viewer.viewList) {
      const profiles = this._getViewProfiles(view);
      const existing = AdaptiveQuality.getFor(profiles);
      if (!existing?.enabled) continue;
      existing.destroy();
      new AdaptiveQuality({viewProfiles: profiles, restMs: this._restMs});
    }
  }
  ...
  private _buildViewRow(view: View): HTMLElement {
    ...
    const profiles = this._getViewProfiles(view);
    toggle.checked = AdaptiveQuality.getFor(profiles)?.enabled === true;
  ...
  private _getViewProfiles(view: View): ViewProfiles {
    let profiles = this._viewProfiles.get(view);
    if (!profiles) {
      profiles = new ViewProfiles(view, {
        profiles: DEFAULT_VIEW_PROFILES,
        activeProfile: "realistic"
      });
      this._viewProfiles.set(view, profiles);
    }
    return profiles;
  }
