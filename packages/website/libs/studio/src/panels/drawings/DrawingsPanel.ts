 ...
 *
 * Same chrome / lifecycle as the sister panels: per-Studio
...
import {
  ...
  type DrawingClipSpec,
} from "@xeokit/website-presentations/drawings";
...
export interface DrawingsPanelParams {
  /**
   * Studio whose Scene the panel manages drawings for.
...
interface DrawingConfig {
  ...
  tileSize: number;
  /**
   * When true, transparent source meshes (effective opacity
   * below 0.99) are drawn as wireframe only — they don't
   * occlude in the HLE depth buffer and don't produce fills,
   * but their edges are still emitted. Lets a window's frame
   * and pane outlines appear in the drawing while showing the
   * room interior through the glass.
   */
  transparentAsWireframe: boolean;
  /**
   * When true, every IfcSpace (or other configured data type)
   * with a name gets a label rendered inside its projected
   * fill polygon at its pole of inaccessibility. Requires the
   * panel's Studio to have a populated Data graph and the
   * drawing's `fill` style to be on (which the panel's "Solid
   * fill" toggle already drives — wireframe-only mode skips
   * labels because there are no fills to anchor them inside).
   */
  spaceLabels: boolean;
  /**
...
export class DrawingsPanel extends FloatingPanelBase {
   ...
   * alive for GC.
   */
  private static readonly _instances = new WeakMap<Studio, DrawingsPanel>();
  ...
  static getFor(studio: Studio): DrawingsPanel | undefined {
    const inst = DrawingsPanel._instances.get(studio);
  ...
  static openFor(params: DrawingsPanelParams): DrawingsPanel {
    let inst = DrawingsPanel._instances.get(params.studio);
  ...
  /**
   * Debounce handle for SectionPlane-driven re-projections. The
   * viewer fires pos / dir change events on every frame while a
   * gizmo is being dragged; we coalesce those into a single
   * `_apply` per ~150 ms of quiescence rather than re-projecting
   * the whole model on every pointermove.
   */
  private _sectionPlaneTimer: ReturnType<typeof setTimeout> | null = null;

  constructor(params: DrawingsPanelParams) {
    if (!params?.studio) {
      throw new Error("DrawingsPanel: studio is required");
    }
    super({
    ...
    this._viewSpecs = computeViewSpecs(this.studio);

    const prior = DrawingsPanel._instances.get(this.studio);
    if (prior && !prior._destroyed) prior.destroy();
    DrawingsPanel._instances.set(this.studio, this);
  ...
  destroy(): void {
    ...
    for (const id of this._ownedProjectionIds) {
      clearDrawing(this.studio.scene.models?.[id]);
    }
    this._ownedProjectionIds.clear();
    this._configs.clear();
    if (this._sectionPlaneTimer != null) {
      clearTimeout(this._sectionPlaneTimer);
      this._sectionPlaneTimer = null;
    }
    for (const u of this._unsubs) {
      try { u(); } catch { /* ignore */ }
    }
    this._unsubs.length = 0;
    if (DrawingsPanel._instances.get(this.studio) === this) {
      DrawingsPanel._instances.delete(this.studio);
    }
    super.destroy();
  }
  ...
  private _attachSubscriptions(): void {
    ...
    // SectionPlane changes — re-project every enabled drawing so
    // the cuts visible in the View are mirrored in the drawing.
    // Coalesced through `_scheduleSectionPlaneRefresh` so an
    // active gizmo drag becomes one re-project, not sixty.
    const viewerEv = (this.studio.viewer as any)?.events;
    if (viewerEv) {
      const refresh = () => this._scheduleSectionPlaneRefresh();
      if (viewerEv.onSectionPlaneCreated?.subscribe)    sub(viewerEv.onSectionPlaneCreated.subscribe(refresh));
      if (viewerEv.onSectionPlaneDestroyed?.subscribe)  sub(viewerEv.onSectionPlaneDestroyed.subscribe(refresh));
      if (viewerEv.onSectionPlanePosChanged?.subscribe) sub(viewerEv.onSectionPlanePosChanged.subscribe(refresh));
      if (viewerEv.onSectionPlaneDirChanged?.subscribe) sub(viewerEv.onSectionPlaneDirChanged.subscribe(refresh));
      if (viewerEv.onSectionPlaneActive?.subscribe)     sub(viewerEv.onSectionPlaneActive.subscribe(refresh));
    }
  }

  /**
   * Coalesce SectionPlane change events into one re-project per
   * ~150 ms of quiescence. While a gizmo is being dragged the
   * viewer emits pos/dir changes on every frame; rebuilding a
   * full drawing each tick would dominate the main thread and
   * push the gizmo's own UI thread off its budget. 150 ms is
   * long enough that the drag settles before a re-project runs,
   * short enough that the drawing feels live once the user
   * releases.
   */
  private _scheduleSectionPlaneRefresh(): void {
    if (this._destroyed) return;
    if (this._sectionPlaneTimer != null) {
      clearTimeout(this._sectionPlaneTimer);
    }
    this._sectionPlaneTimer = setTimeout(() => {
      this._sectionPlaneTimer = null;
      if (this._destroyed) return;
      for (const [modelId, cfg] of this._configs) {
        if (cfg.enabled) void this._apply(modelId);
      }
    }, 150);
  }
  ...
  private _listSourceModels(): SceneModel[] {
    ...
    for (const id of Object.keys(models)) {
      ...
      // Whitelist the default ViewLayer only. Named-layer
      // models — tool overlays (TransformControls, section-plane
      // proxies, NavCube), user annotations, comparison
      // snapshots, … — are skipped, since none of them are
      // valid projection sources for a drawing.
  ...
  private _renderModelRow(model: SceneModel): HTMLElement {
    ...
    detail.appendChild(el("div", "xkt-bp-detail-label", {textContent: "Transparent"}));
    const xpLabel = el("label", "xkt-bp-view-check");
    const xpCb = el("input") as HTMLInputElement;
    xpCb.type = "checkbox";
    xpCb.checked = cfg.transparentAsWireframe;
    xpCb.title = "Draw transparent meshes (e.g. window glass) as wireframe only — their edges still emit, but they don't occlude geometry behind them in the depth buffer and don't produce fills.";
    xpCb.addEventListener("change", () => {
      cfg.transparentAsWireframe = xpCb.checked;
      this._apply(model.id);
    });
    xpLabel.append(xpCb, document.createTextNode("Glass as wireframe"));
    detail.appendChild(xpLabel);

    detail.appendChild(el("div", "xkt-bp-detail-label", {textContent: "Labels"}));
    const labelsRow = el("label", "xkt-bp-view-check");
    const labelsCb = el("input") as HTMLInputElement;
    labelsCb.type = "checkbox";
    labelsCb.checked = cfg.spaceLabels;
    labelsCb.title = "Render an interior label (room name) for every IfcSpace in the drawing. Requires the source model's Data graph and the drawing's fill style to be on.";
    labelsCb.addEventListener("change", () => {
      cfg.spaceLabels = labelsCb.checked;
      this._apply(model.id);
    });
    labelsRow.append(labelsCb, document.createTextNode("Room labels"));
    detail.appendChild(labelsRow);
  ...
  private async _apply(modelId: string): Promise<void> {
    ...
    for (const spec of this._viewSpecs) {
      ...
      //      world-space `pos` (a point on the plane) and `dir`
      //      (the normal of the *discarded* half-space). The
      //      drawing pipeline's clip spec is the inverse — its
      //      `normal` points toward the *kept* side — so we
      //      negate `dir` when translating.
      //
      // Multiple cuts intersect: a fragment must be on the kept
      // side of every supplied plane to survive rasterisation.
      // Empty result → no clipping, same as the original
      // unsectioned drawing.
      const clipSpecs: DrawingClipSpec[] = [];
      if (cfg.clipEnabled) {
        clipSpecs.push({depth: cfg.clipDepth});
      }
      const viewRec = Object.values(this.studio.viewManager.views ?? {})[0];
      const sectionPlanes = viewRec?.view?.sectionPlanesList;
      if (sectionPlanes) {
        for (const plane of sectionPlanes) {
          if (!plane.active) continue;
          clipSpecs.push({
            point:  [plane.pos[0],  plane.pos[1],  plane.pos[2]],
            normal: [-plane.dir[0], -plane.dir[1], -plane.dir[2]],
          });
        }
      }
      ...
      const result = await buildDrawing({
        ...
        ...(clipSpecs.length > 0 ? {clip: clipSpecs} : {}),
        // When on, glass / window meshes draw as wireframe and
        // don't occlude the geometry behind them.
        ...(cfg.transparentAsWireframe ? {transparentAsWireframe: true} : {}),
        // Room labels — placed inside each IfcSpace's projected
        // fill polygon at its pole of inaccessibility. The
        // label pass reads names + types from the helper's
        // Data graph; we only thread it in when labels are on
        // so non-label drawings stay decoupled from `data`.
        ...(cfg.spaceLabels && this.studio.data
          ? {data: this.studio.data, spaceLabels: true}
          : {}),
      });
      if (result.ok === false) {
  ...
  private _teardown(modelId: string): void {
    for (const spec of this._viewSpecs) {
      ...
      clearDrawing(this.studio.scene.models?.[targetId]);
 ...
 */
function computeViewSpecs(studio: Studio): ViewSpec[] {
  const cs = (studio.scene as any)?.coordinateSystem;
