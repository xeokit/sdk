 ...
 * per-Studio WeakMap registry, idempotent `getFor` /
...
import {XGFExporter}              from "@xeokit/sdk/formats/xgf/XGFExporter";
import {XGFStreamExporter}        from "@xeokit/sdk/formats/xgfstream/XGFStreamExporter";
import {GLTFExporter}             from "@xeokit/sdk/formats/gltf/GLTFExporter";
import {FBXExporter}              from "@xeokit/sdk/formats/fbx/FBXExporter";
import {USDZExporter}             from "@xeokit/sdk/formats/usdz/USDZExporter";
import {GaussianSplatExporter}    from "@xeokit/sdk/formats/gaussiansplat/GaussianSplatExporter";
import {OBJExporter}              from "@xeokit/sdk/formats/obj/OBJExporter";
import {PLYExporter}              from "@xeokit/sdk/formats/ply/PLYExporter";
import {MTLExporter}              from "@xeokit/sdk/formats/mtl/MTLExporter";
import {IFCExporter}              from "@xeokit/sdk/formats/ifc/IFCExporter";
import {DotBIMExporter}           from "@xeokit/sdk/formats/dotbim/DotBIMExporter";
import {CityJSONExporter}         from "@xeokit/sdk/formats/cityjson/CityJSONExporter";
import {DXFExporter}              from "@xeokit/sdk/formats/dxf/DXFExporter";
import {SVGExporter}              from "@xeokit/sdk/formats/svg/SVGExporter";
import {ThreeDXMLExporter}        from "@xeokit/sdk/formats/threedxml/ThreeDXMLExporter";
import {XKTExporter}              from "@xeokit/sdk/formats/legacy/xkt/XKTExporter";
import {SceneModelExporter} from "@xeokit/sdk/formats/scenemodel/SceneModelExporter";
import {DataModelExporter}  from "@xeokit/sdk/formats/datamodel/DataModelExporter";
import {MetaModelExporter}  from "@xeokit/sdk/formats/legacy/metamodel/MetaModelExporter";
...
export class ExportDialog extends FloatingPanelBase {
  ...
  /** Per-Studio instance registry. */
  private static readonly _instances = new WeakMap<Studio, ExportDialog>();
  ...
  static getFor(studio: Studio): ExportDialog | undefined {
    const inst = ExportDialog._instances.get(studio);
    return inst && !inst._destroyed ? inst : undefined;
  }

  static openFor(params: ExportDialogParams): ExportDialog {
    let inst = ExportDialog._instances.get(params.studio);
  ...
  constructor(params: ExportDialogParams) {
    if (!params || !params.studio) {
      throw new Error("ExportDialog: studio is required");
    }
    super({
    ...
    const prior = ExportDialog._instances.get(params.studio);
    if (prior && !prior._destroyed) prior.destroy();
    ExportDialog._instances.set(params.studio, this);
  ...
  destroy(): void {
    if (this._destroyed) return;
    this._detachListeners();
    if (ExportDialog._instances.get(this.studio) === this) {
      ExportDialog._instances.delete(this.studio);
    }
    super.destroy();
  }
  ...
  private _attachListeners(): void {
    ...
    const sceneEv: any = (this.studio.scene as any).events;
    if (sceneEv) {
    ...
    const dataEv: any = (this.studio.data as any).events;
    if (dataEv) {
  ...
  private _renderModels(): void {
    ...
    const sceneModels = this.studio.scene.models as Record<string, SceneModel>;
    const dataModels  = this.studio.data.models as Record<string, DataModel>;

    // SceneModels in named ViewLayers (tool overlays,
    // annotations, comparisons, …) aren't user-authored model
    // content — skip them from the exporter's row list.
    const idSet = new Set<string>();
    if (sceneModels) {
      for (const id of Object.keys(sceneModels)) {
        if (isDefaultLayerModel(sceneModels[id])) idSet.add(id);
      }
    }
  ...
  private _isDatasetCompatible(desc: string): boolean {
    ...
    const sm = (this.studio.scene.models as any)?.[modelId];
    const dm = (this.studio.data.models  as any)?.[modelId];
...
function downloadBlob(blob: Blob, filename: string): void {
  ...
  try {
    ...
    // we revoke — matches the snapshot-export helpers in Studio.
