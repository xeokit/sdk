...
import type {CoordinateSystemParams, SceneModelUpdateHint} from "@xeokit/sdk/model/scene";
...
export class ImportDialog extends FloatingPanelBase {
  ...
  private static readonly _instances = new WeakMap<Studio, ImportDialog>();
  ...
  static getFor(studio: Studio): ImportDialog | undefined {
    const inst = ImportDialog._instances.get(studio);
    return inst && !inst._destroyed ? inst : undefined;
  }

  static openFor(params: ImportDialogParams): ImportDialog {
    let inst = ImportDialog._instances.get(params.studio);
  ...
  constructor(params: ImportDialogParams) {
    ...
    const prior = ImportDialog._instances.get(this.studio);
    if (prior && !prior._destroyed) prior.destroy();
    ImportDialog._instances.set(this.studio, this);
  ...
  destroy(): void {
    ...
    if (ImportDialog._instances.get(this.studio) === this) {
      ImportDialog._instances.delete(this.studio);
    }
    super.destroy();
  }
  ...
  private async _runLoad(): Promise<void> {
    ...
    try {
      if (this._activeDataSet.onLoad) {
        this._activeDataSet.onLoad(this.studio);
      } else {
