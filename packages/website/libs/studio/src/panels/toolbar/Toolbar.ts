...
import {VehicleNavigationController} from "@xeokit/sdk/viewing/navigation/vehicle";
...
export class Toolbar extends FloatingPanelBase {
  ...
  private _makeActionCtx(): ToolbarActionContext {
    return {
      ...
      modelNavigation:() => this._modelNavigation(),
  ...
  /**
   * Look up the {@link ModelNavigationController} the {@link Studio}
   * registered alongside the active View. Same shape as
   * {@link _cameraFlight}: the `ModelNavigationController` is not a property
   * of {@link viewing!viewer.View | View} itself —
   * `ViewManager.createView()` stashes it on
   * `studio.viewManager.views[viewId].modelNavigation`. Returns
   * `null` when no helper is wired or the active View has no
   * registered controller.
   */
  private _modelNavigation(): ModelNavigationController | null {
    const view = this._activeView();
    if (!view || !this.studio) return null;
    const record = (this.studio.viewManager.views as any)?.[view.id];
    return record?.modelNavigation ?? null;
  }
