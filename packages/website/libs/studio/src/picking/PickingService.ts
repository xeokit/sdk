...
import {type PickParams, PickResult, type View} from "@xeokit/sdk/viewing/viewer";
import type {SDKResult} from "@xeokit/sdk/base/core";
import {SceneCollisionIndex} from "@xeokit/sdk/spatial/collision";
import {MemoisingPickStrategy, RoutingPickStrategy, type PickStrategy} from "@xeokit/sdk/spatial/picking";
...
export class PickingService {
   ...
   * `pointermove` → ModelNavigationController hover + measurement hover both
  ...
  /**
   * ModelNavigationController-shape pick adapter.
   *
   * Adapts the viewer-side `PickParams`/`PickResult` contract that
   * ModelNavigationController and PickController expect onto the unified
   * `PickStrategy.pick` interface. Snap fields (`snapToVertex` /
   * `snapToEdge`) are forwarded so any ModelNavigationController consumer that
   ...
   *
   * Use this as the `pick` callback when constructing a ModelNavigationController;
   * for one-off canvas-position picks reach for {@link picker} directly.
   */
  pickForModelNavigationController(view: View, pickParams: PickParams): SDKResult<PickResult> {
