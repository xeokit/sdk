...
import type {DataModel, DataObject} from "@xeokit/sdk/model/data";
...
import type {LoaderProgress} from "@xeokit/sdk/formats/LoaderProgress";
...
export async function applyIFCMaterials(params: {
  ...
  const ensureMaterial = (ifcType: string): string | null => {
    ...
    const matRes = sceneModel.createMaterial({
      ...
      // SceneMaterial as `color` / `opacity`. The drawings
      // emitter (and any other textureless consumer) reads
      // these via `mesh.effectiveColor` / `effectiveOpacity`, so
      // a glass painter's `flatOpacity: 0.35` survives into the
      // plan as a translucent fill.
      //
      // Per-type `entry.material` wins — IFC entries like
      // IfcWindow / IfcCurtainWall set their own `color +
      // opacity` for tint and BLEND, and those choices must
      // override the painter's default.
