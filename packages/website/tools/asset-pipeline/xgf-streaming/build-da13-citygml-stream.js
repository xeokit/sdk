...
// Converts the DA13 CityGML buildings file into a localized, meter-based XGF stream.
...
const MODEL_ID = "DA13_3D_Buildings_Merged";
const DEFAULT_INPUT = "/home/lindsay/Downloads/DA13_3D_Buildings_Merged.gml";
const US_SURVEY_FOOT_TO_METER = 1200 / 3937;
...
const LOCAL_SOURCE_COORDINATE_SYSTEM = {
    ...
    origin: [0, 0, 0],
    units: "feet",
    scaleToMeters: US_SURVEY_FOOT_TO_METER
};

const MATERIALS = {
    roof: {
        id: "da13-roof",
        color: [0.55, 0.16, 0.12],
        roughness: 0.9
    },
    wall: {
        id: "da13-wall",
        color: [0.66, 0.58, 0.49],
        roughness: 0.82
    },
    ground: {
        id: "da13-ground",
...
async function main() {
    ...
    const chunkBudget = parsePositiveInteger(args.chunkBudget, 1200);
    const minChunkBudget = parsePositiveInteger(args.minChunkBudget, 300);
    const gridCellSize = parsePositiveNumber(args.gridCellSize, 1200);
    ...
    const sourceCoordinateSystem = {
        crs: envelope.srsName || "EPSG:2263",
        basis: SDK_DEFAULT_COORDINATE_SYSTEM.basis.slice(),
    ...
    console.log(`[DA13] Reading ${inputPath}`);
    console.log(`[DA13] Source ${sourceCoordinateSystem.crs}, local origin ${sourceOrigin.join(", ")}`);
    ...
    console.log(`[DA13] Parsed ${parseStats.objects} objects, ${parseStats.meshes} meshes, ${parseStats.polygons} polygons`);
    console.log(`[DA13] Exporting XGF stream to ${streamDir}`);
    ...
    console.log(`[DA13] Wrote ${Object.keys(stream.files).length} stream files`);
    console.log(`[DA13] Runtime chunks: ${chunkCount}`);
}
...
function printHelp() {
    console.log(`Usage:
  node packages/website/tools/asset-pipeline/xgf-streaming/build-da13-citygml-stream.js [gml-file]
...
Options:
  ...
  --chunk-budget <count>     Target meshes per stream chunk. Defaults to 1200
  --min-chunk-budget <count> Minimum meshes per stream chunk. Defaults to 300
  --grid-cell-size <feet>    Grid partition cell size in localized source feet. Defaults to 1200
`);
}
...
function flushBuilding(state) {
    ...
    if (state.stats.objects % 1000 === 0) {
        console.log(`[DA13] Parsed ${state.stats.objects} renderable buildings`);
    }
}
