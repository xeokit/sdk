...
export async function main(argv = process.argv.slice(2)): Promise<void> {
  ...
  const output = path.resolve(String(args.output || path.join(rootDir, "packages/website/tools/city-generator/src/profiles", `${name}.json`)));
...
function printHelp(): void {
  console.log(`Usage:
  node packages/website/tools/city-generator/analyze-osm.mjs --input <extract> [options]
