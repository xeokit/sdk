...
import {XGFExporter as SDKXGFExporter} from "@xeokit/sdk/formats/xgf";
import {XGFStreamExporter as SDKXGFStreamExporter, type XGFStreamChunkRepSetOptions} from "@xeokit/sdk/formats/xgfstream";
...
export async function exportXGFStream(cityScene: CityScene, options: {
  ...
  const stream = await new SDKXGFStreamExporter().write({sceneModel}, {
    ...
    chunkRepSets: options.chunkRepSets !== false
      ? {
        idPrefix: "procedural-city-chunk-lod",
        allRepId: "all",
        regularRepId: "regular",
        dominantRepId: "dominant",
        allMinPixels: 1000,
        regularMinPixels: 640,
        regularMaxPixels: 1000,
        dominantMaxPixels: 640,
        hysteresisPixels: 96,
        dominantObjectIds: cityScene.objects
          .filter((object) => object.metadata?.lodRole === "dominant")
          .map((object) => object.id),
        regularObjectIds: cityScene.objects
          .filter((object) => object.metadata?.lodRole === "dominant" || object.metadata?.lodRole === "regular")
          .map((object) => object.id),
        ...(typeof options.chunkRepSets === "object" ? options.chunkRepSets : {})
      }
      : false,
    yieldIntervalMs: 80
  });
...
export function buildSceneModel(cityScene: CityScene): {scene: Scene; sceneModel: any} {
