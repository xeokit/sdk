...
import {exportThreeDTiles} from "./export/ThreeDTilesExporter";
...
export async function main(argv = process.argv.slice(2)): Promise<void> {
  ...
  const threeDTiles = args["3dtiles"] === true
    || args.tiles === true
    || (typeof args.output === "string" && path.basename(args.output) === "threedtiles");
  const defaultOutput = threeDTiles
    ? path.join(rootDir, "packages/website/models/ProceduralCityAmsterdam3DTiles/threedtiles")
    : stream
  ...
  const defaultSize = threeDTiles ? 1600 : stream ? 2400 : 1000;
  const defaultBuildings = threeDTiles ? 1800 : stream ? 3600 : 560;
  const city = await generateCity({
    seed: args.seed ?? 42,
    size: numberArg(args.size, defaultSize),
    style: "european",
    density: (args.density === "medium" ? "medium" : "high"),
    buildingCount: args.buildings || stream || threeDTiles ? numberArg(args.buildings, defaultBuildings) : undefined,
    profile: typeof args.profile === "string" ? args.profile : undefined,
    outputPath: stream ? path.join(outputPath, "index.runtime.json") : threeDTiles ? path.join(outputPath, "tileset.json") : outputPath,
  ...
  const threeDTilesResult = threeDTiles ? await exportThreeDTiles(city, {
    outputDir: outputPath,
    gridCellSize: numberArg(args["grid-cell-size"], 260),
    maxObjectsPerTile: numberArg(args["tile-object-budget"], 420)
  }) : undefined;
  if (!stream && !threeDTiles) {
    await exportXGF(city, {outputPath});
  }
  const sidecarDir = sidecarDirectory(outputPath, stream || threeDTiles);
  ...
  const defaultReportPath = path.join(rootDir, "artifacts/evaluation", `${reportSlug(city.config.profileData?.name || "city", city.config.seed, threeDTiles ? "3dtiles" : stream ? "large-stream" : undefined)}.json`);
  ...
  const evaluationArtifactPath = path.join(rootDir, "artifacts/evaluation", `${reportSlug(city.config.profileData?.name || "city", city.config.seed, threeDTiles ? "3dtiles" : stream ? "large-stream" : undefined)}.evaluation.json`);
  ...
  } else if (threeDTilesResult) {
    console.log(`3D Tiles content: ${threeDTilesResult.contentCount}`);
    console.log(`3D Tiles grid tiles: ${threeDTilesResult.tileCount}`);
    console.log(`Wrote ${path.relative(rootDir, path.join(outputPath, "tileset.json"))}`);
  } else {
...
function printHelp(): void {
  console.log(`Usage:
  node packages/website/tools/city-generator/build-city.mjs [options]
...
Options:
  ...
  --3dtiles            Export an explicit 3D Tiles 1.1 tileset with GLB content
  ...
  --chunk-budget <n>   XGF stream chunk mesh budget, default 420
  --min-chunk-budget <n> Minimum merged XGF stream chunk budget, default 120
  --tile-object-budget <n> Maximum objects per 3D Tiles GLB tile, default 420
  --grid-cell-size <m> Stream or tile grid cell size, default 260
`);
}
...
function sidecarDirectory(outputPath: string, stream = false): string {
  if (stream) {
    ...
    return base === "xgfstream" || base === "threedtiles" ? path.dirname(outputPath) : outputPath;
  }
