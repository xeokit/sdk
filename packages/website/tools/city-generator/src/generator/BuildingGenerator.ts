...
export function generateBuildings(parcels: Parcel[], config: CityGeneratorConfig, streams: RandomStreams, urbanContext?: CityUrbanContext): CityObject[] {
  ...
  return selected.flatMap((parcel, index) => createBuilding(parcel, index + 1, config, streams, urbanContext));
}
...
function createBuilding(parcel: Parcel, ordinal: number, config: CityGeneratorConfig, streams: RandomStreams, urbanContext?: CityUrbanContext): CityObject[] {
  ...
  const massMeshes: CityObject["meshes"] = [];
  const detailMeshes: CityObject["meshes"] = [];
  ...
  if (massing === "podium-tower") {
    ...
    massMeshes.push(extrudePolygon({polygon: footprint, height: podiumHeight, materialId: "concrete"}));
    addDetailMeshes(detailMeshes, footprint, 0, podiumHeight, podiumFloors, parcel, usage, "concrete-grid", roofType, streams, buildingContext);
    ...
    massMeshes.push(extrudePolygon({polygon: towerFootprint, height: towerHeight, baseZ: podiumHeight, materialId: wallMaterialId}));
    addDetailMeshes(detailMeshes, towerFootprint, podiumHeight, towerHeight, floors - podiumFloors, parcel, usage, "curtain-wall", "flat", streams, buildingContext);
    ...
    for (const part of parts) {
      massMeshes.push(extrudePolygon({polygon: part, height, materialId: wallMaterialId}));
      addDetailMeshes(detailMeshes, part, 0, height, floors, parcel, usage, facadeStyle, roofType, streams, buildingContext);
    }
    ...
    massMeshes.push(extrudePolygon({polygon: footprint, height: lowerHeight, materialId: wallMaterialId}));
    addDetailMeshes(detailMeshes, footprint, 0, lowerHeight, lowerFloors, parcel, usage, facadeStyle, "terrace", streams, buildingContext);
    const upper = scalePolygon(footprint, 0.78, 0.74);
    const upperHeight = height - lowerHeight;
    massMeshes.push(extrudePolygon({polygon: upper, height: upperHeight, baseZ: lowerHeight, materialId: wallMaterialId}));
    addDetailMeshes(detailMeshes, upper, lowerHeight, upperHeight, floors - lowerFloors, parcel, usage, facadeStyle, roofType, streams, buildingContext);
  } else {
    massMeshes.push(extrudePolygon({polygon: footprint, height, materialId: wallMaterialId}));
    addDetailMeshes(detailMeshes, footprint, 0, height, floors, parcel, usage, facadeStyle, roofType, streams, buildingContext);
  }
  ...
  if (detailMeshes.length > 0) {
    objects.push({
      id: `${id}-details`,
      name: `${parcel.district} ${usage} ${ordinal} details`,
      type: "BuildingDetail",
      layerId: "buildings",
      meshes: detailMeshes,
      metadata: {
