...
const ROAD_SURFACE_Z = 0.115;
const SIDEWALK_SURFACE_Z = 0.045;
const ROAD_SURFACE_LAYER_STEP = 0.018;
const ROAD_SURFACE_END_EXTENSION = 0;

export function generateRoadObjects(network: RoadNetwork): CityObject[] {
  return network.roads.map((road, roadIndex) => {
    const asphalt = new MeshBuilder();
    const sidewalks = new MeshBuilder();
    const roadZ = roadSurfaceZ(road, roadIndex);
    const sidewalkZ = sidewalkSurfaceZ(road, roadIndex);
    ...
    for (let i = 1; i < road.polyline.length; i++) {
      ...
      for (const [spanA, spanB] of roadSpans) {
        asphalt.addRoadSegment(spanA, spanB, road.width, roadZ, ROAD_SURFACE_END_EXTENSION);
        visibleSegments++;
      }
      for (const [spanA, spanB] of sidewalkSpans) {
        sidewalks.addRoadSegment(spanA, spanB, road.width + 7, sidewalkZ, ROAD_SURFACE_END_EXTENSION);
      }
    }
    return {
      id: road.id,
...
function roadSurfaceZ(road: Road, index: number): number {
  const hierarchyLayer = road.hierarchy === "arterial" ? 3 : road.hierarchy === "collector" ? 2 : road.hierarchy === "pedestrian" ? 1 : 0;
  return ROAD_SURFACE_Z + (hierarchyLayer * 2 + orientationLayer(road) + (index % 3) * 0.25) * ROAD_SURFACE_LAYER_STEP;
}

function sidewalkSurfaceZ(road: Road, index: number): number {
  return SIDEWALK_SURFACE_Z + (orientationLayer(road) + (index % 2) * 0.25) * ROAD_SURFACE_LAYER_STEP;
}

function orientationLayer(road: Road): number {
  const first = road.polyline[0];
  const last = road.polyline[road.polyline.length - 1];
  if (!first || !last) {
    return 0;
  }
  return Math.abs(last[0] - first[0]) >= Math.abs(last[1] - first[1]) ? 0 : 1;
}
