...
export const COUNTRY_BOUNDARY_LAYER_ID = "countryBoundaries";

export interface CountryBoundaryBuildResult {
  lines: LineData[];
  segments: number;
}

export function buildCountryBoundaries(features: LineFeature[], options: EarthOptions): CountryBoundaryBuildResult {
    ...
    for (const {feature, bbox} of indexed) {
      ...
      for (const clipped of lineSegmentsForTile(feature, tile)) {
        ...
        for (let i = 0; i < dense.length; i++) {
          const [x, y, z] = lonLatToXYZ(dense[i], options.earthRadius + options.landOffset + options.boundaryOffset);
          positions[i * 3] = x;
          positions[i * 3 + 1] = y;
          positions[i * 3 + 2] = z;
          if (i + 1 < dense.length) {
        ...
        lines.push({
          id: `earth.countryBoundary.tile.${tileId(tile)}.${String(part++).padStart(4, "0")}`,
          positions,
          indices,
          materialId: "earth.countryBoundary",
          layerId: COUNTRY_BOUNDARY_LAYER_ID
        });
      }
    }
  }
