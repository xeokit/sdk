...
import {View} from "./scene/renderer/View.js";

/**
 * The 3D Viewer at the heart of the xeokit SDK.
 *
 * * Has a single {@link Scene}.
 * * Has one or more {@link View}s, each of which has a {@link Canvas} and a {@link Camera}.
 * * Add {@link Plugin}s to a Viewer to extend its functionality.
 * * {@link Viewer#metaScene} holds metadata about models in the Viewer's {@link MetaScene}.
 * * Use {@link Viewer#cameraFlight} to fly or jump the {@link Scene}'s {@link Camera} to target positions, boundaries or {@link Entity}s.
 *
 * @public
 */
class Viewer {
    ...
    constructor(cfg) {
        ...
        /**
         * The {@link View}s attached to this Viewer.
         *
         * Each View renders the {@link Scene} to a separate {@link Canvas}, via a separate {@link Camera}.
         *
         * @property views
         * @type {{String:View}}
         * @readonly
        ...
        if (cfg.views) {
            for (let i = 0, len = cfg.views.length; i < len; i++) {
                this.createView(cfg.views[i]);
            }
        }
    }

    /**
     * Creates a {@link View} on this Viewer.
     * @param cfg
     * @return {View}
     */
    createView(cfg) {
        const view = new View(this.scene, cfg);
