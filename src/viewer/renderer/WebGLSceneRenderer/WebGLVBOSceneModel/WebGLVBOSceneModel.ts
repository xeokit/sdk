...
import {WebGLVBOMesh} from './lib/WebGLVBOMesh';
...
import {worldToRTCPositions} from "../../../viewer/math/rtc/index";
import {WebGLVBOGeometry} from "./lib/WebGLVBOGeometry";
import {SceneModel} from "../../../viewer/scene/SceneModel";
import {RenderFlags} from "./lib/RenderFlags";
import {WebGLVBOTextureSet} from "./lib/WebGLVBOTextureSet";
import {getScratchMemory, putScratchMemory, ScratchMemory} from "./lib/ScratchMemory";
import {WebGLVBOTexture} from "./lib/WebGLVBOTexture";
import {Texture2D} from "../lib/Texture2D";
import {Events} from "../../../viewer/Events";
import {Transform} from "../../viewer/../scene/Transform";
import {createUUID, loadArraybuffer} from "../../utils/index";
import {WebGLVBOSceneObject} from "./lib/WebGLVBOSceneObject";
import {TrianglesBatchingLayer} from "./lib/layers/trianglesBatching/TrianglesBatchingLayer";
import {LinesBatchingLayer} from "./lib/layers/linesBatching/LinesBatchingLayer";
import {WebGLSceneRendererDrawable} from "./../lib/WebGLSceneRendererDrawable";
import {collapseAABB3, expandAABB3} from "../../../viewer/math/boundaries";
import {buildEdgeIndices} from "../../../viewer/math/geometry/index";
import {GeometryCfg, MeshCfg, SceneObjectCfg, TextureCfg, TextureSetCfg} from "../../../viewer/index";
...
class WebGLVBOSceneModel extends Component implements SceneModel, WebGLSceneRendererDrawable {
    ...
    sceneObjects: { [key: string]: WebGLVBOSceneObject };
    sceneObjectList: WebGLVBOSceneObject[];
    numPortions: number;
    ...
    #layerList: any[];
    #textures: { [key: string]: WebGLVBOTexture };
    #textureSets: { [key: string]: WebGLVBOTextureSet };
    #geometries: { [key: string]: WebGLVBOGeometry };
    #meshes: { [key: string]: WebGLVBOMesh };
    ...
    private _lastUVDecompressMatrix: FloatArrayType;
    ...
    set backfaces(backfaces: boolean) {
        ...
        this.webglSceneRenderer.imageDirty();
    }
    ...
    _createDefaultTextureSet() {
        ...
        const defaultColorTexture = new WebGLVBOTexture({
        ...
        const defaultMetalRoughTexture = new WebGLVBOTexture({
        ...
        const defaultNormalsTexture = new WebGLVBOTexture({
        ...
        const defaultEmissiveTexture = new WebGLVBOTexture({
        ...
        const defaultOcclusionTexture = new WebGLVBOTexture({
        ...
        this.#textureSets[defaultTextureSetId] = new WebGLVBOTextureSet({
    ...
    createGeometry(cfg: GeometryCfg): void {
        ...
        if (cfg.uvCompressed && !cfg.uvDecompressMatrix) {
            this.scene.error("Param expected: `uvDecompressMatrix` (required for `uvCompressed')");
            return null;
        }
        ...
        const geometry = new WebGLVBOGeometry(this, this.webglSceneRenderer.gl, cfg);
    ...
    createTexture(cfg: TextureCfg): void {
        ...
        this.#textures[textureId] = new WebGLVBOTexture({id: textureId, texture});
    }

    createTextureSet(cfg: TextureSetCfg) {
        ...
        const textureSet = new WebGLVBOTextureSet({
            id: textureSetId,
            colorTexture,
    ...
    createMesh(cfg: MeshCfg): void {
        ...
        if (sharingGeometry) {
            ...
            const cfgOrigin: FloatArrayType = (cfg.origin || cfg.rtcCenter);
            ...
            if (cfg.uvsDecompressMatrix) {
                if (!this._lastUVDecompressMatrix) {
                    needNewBatchingLayers = true;
                    this._lastUVDecompressMatrix = math.mat4(cfg.uvsDecompressMatrix);

                } else {
                    if (!math.compareMat4(this._lastUVDecompressMatrix, cfg.uvsDecompressMatrix)) {
                        needNewBatchingLayers = true;
                        // @ts-ignore
                        this._lastUVDecompressMatrix.set(cfg.uvsDecompressMatrix)
                    }
                }
            }
            if (cfg.textureSetId) {
    ...
    createSceneObject(cfg: SceneObjectCfg) {
        ...
        const sceneObject = new WebGLVBOSceneObject({
            id,
            sceneModel: this,
            meshes,
            aabb
    ...
    _getActiveSectionPlanesForLayer(layer: any) {
        ...
        const sectionPlanes = this.view.#sectionPlanesState.sectionPlanes;
