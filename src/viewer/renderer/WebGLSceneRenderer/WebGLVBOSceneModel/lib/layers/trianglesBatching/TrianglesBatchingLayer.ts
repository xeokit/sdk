...
import {RenderState} from "../../../../../../utils/index";
...
import {WebGLVBOTextureSet} from "../../WebGLVBOTextureSet";
import {
    compressUVs,
    decompressPosition,
    FloatArrayType,
...
class TrianglesBatchingLayer {
    ...
    private _batchingRenderers: any;
    private _buffer: TrianglesBatchingBuffer;
    #scratchMemory: any;
    private state: any;
    ...
    private _preCompressedNormalsExpected: boolean;
    private aabb: FloatArrayType;
    ...
    constructor(cfg: {
        ...
        this._batchingRenderers = getBatchingRenderers(cfg.sceneModel.scene);
        ...
        if (cfg.uvsDecompressMatrix) {
            this.state.uvsDecompressMatrix = math.mat3(cfg.uvsDecompressMatrix);
    ...
    finalize() {
        ...
        if (buffer.positions.length > 0) {
            ...
            const quantizedPositions = (this.#preCompressedPositionsExpected)
                ...
                : quantizePositions(buffer.positions, this.#modelAABB, state.positionsDecompressMatrix); // BOTTLENECK
    ...
    _setFlags2(portionId: number, flags: number, deferred: boolean = false) {
    ...
    setOffset(portionId: any, offset: FloatArrayType) {
        if (!this.#finalized) {
            throw "Not finalized";
        }
        if (!this.sceneModel.scene.entityOffsetsEnabled) {
    ...
    drawColorOpaque(renderFlags: RenderFlags, frameCtx: FrameContext): void {
        ...
        if (frameCtx.withSAO && this.sceneModel.saoEnabled) {
            if (frameCtx.pbrEnabled && this.sceneModel.pbrEnabled && this.state.pbrSupported) {
            ...
            } else if (frameCtx.colorTextureEnabled && this.sceneModel.colorTextureEnabled && this.state.colorTextureSupported) {
    ...
    _updateBackfaceCull(renderFlags: RenderFlags, frameCtx: FrameContext): void {
    ...
    precisionRayPickSurface(portionId: number, worldRayOrigin: FloatArrayType, worldRayDir: FloatArrayType, worldSurfacePos: { set: (arg0: any) => void; }, worldNormal: any) {
        ...
        math.transformRay(this.sceneModel.worldNormalMatrix, rtcRayOrigin, rtcRayDir, rtcRayOrigin, rtcRayDir); // RTC -> local
        ...
        for (let i = 0, len = indices.length; i < len; i += 3) {
            ...
            decompressPosition(a, state.positionsDecompressMatrix);
            decompressPosition(b, state.positionsDecompressMatrix);
            decompressPosition(c, state.positionsDecompressMatrix);

            if (rayTriangleIntersect(rtcRayOrigin, rtcRayDir, a, b, c, closestIntersectPos)) {
