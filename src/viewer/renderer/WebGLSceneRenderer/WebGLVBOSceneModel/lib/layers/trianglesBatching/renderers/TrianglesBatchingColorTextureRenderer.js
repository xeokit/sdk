...
class TrianglesBatchingColorTextureRenderer {
    ...
    drawLayer(frameCtx, batchingLayer, renderPass) {
        ...
        gl.uniformMatrix4fv(this.#uPositionsDecodeMatrix, false, state.positionsDecompressMatrix);

        if (this.#uUVDecodeMatrix) {
            gl.uniformMatrix3fv(this.#uUVDecodeMatrix, false, state.uvsDecompressMatrix);
        }

        this.#aPosition.bindArrayBuffer(state.positionsBuf);

        if (this.#aUV) {
            this.#aUV.bindArrayBuffer(state.uvBuf);
        }
        ...
        if (textureSet) {
            if (textureSet.colorTexture) {
                this.#program.bindTexture(this.#uColorMap, textureSet.colorTexture.texture, frameCtx.textureUnit);
        ...
        if (this._withSAO) {
            ...
            if (saoEnabled) {
                ...
                this.#program.bindTexture(this.#uOcclusionTexture, frameCtx.occlusionTexture, frameCtx.textureUnit);
                frameCtx.textureUnit = (frameCtx.textureUnit + 1) % maxTextureUnits;
    ...
    #allocate() {
        ...
        this.#uUVDecodeMatrix = program.getLocation("uvsDecompressMatrix");
