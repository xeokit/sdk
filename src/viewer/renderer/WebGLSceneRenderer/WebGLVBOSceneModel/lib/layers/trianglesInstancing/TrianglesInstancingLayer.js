...
class TrianglesInstancingLayer {
    ...
    constructor(cfg) {
        ...
        this.state = new RenderState(stateCfg);
        ...
        if (cfg.origin) {
            this.state.origin.set(cfg.origin);
        }
    ...
    createPortion(cfg) {
        ...
        const obb = this.state.geometry.obb;
    ...
    finalize() {
        ...
        if (colorsLength > 0) {
            ...
            this.state.colorsBuf = new ArrayBuf(gl, gl.ARRAY_BUFFER, new Uint8Array(this._colors), this._colors.length, 4, gl.DYNAMIC_DRAW, notNormalized);
        ...
        if (this._metallicRoughness.length > 0) {
            ...
            this.state.metallicRoughnessBuf = new ArrayBuf(gl, gl.ARRAY_BUFFER, metallicRoughness, this._metallicRoughness.length, 2, gl.STATIC_DRAW, normalized);
        }
        if (flagsLength > 0) {
        ...
        if (this._modelMatrixCol0.length > 0) {
            ...
            if (this.state.geometry.normalsBuf) {
                this.state.modelNormalMatrixCol0Buf = new ArrayBuf(gl, gl.ARRAY_BUFFER, new Float32Array(this._modelNormalMatrixCol0), this._modelNormalMatrixCol0.length, 4, gl.STATIC_DRAW, normalized);
                this.state.modelNormalMatrixCol1Buf = new ArrayBuf(gl, gl.ARRAY_BUFFER, new Float32Array(this._modelNormalMatrixCol1), this._modelNormalMatrixCol1.length, 4, gl.STATIC_DRAW, normalized);
                this.state.modelNormalMatrixCol2Buf = new ArrayBuf(gl, gl.ARRAY_BUFFER, new Float32Array(this._modelNormalMatrixCol2), this._modelNormalMatrixCol2.length, 4, gl.STATIC_DRAW, normalized);
                this._modelNormalMatrixCol0 = [];
                this._modelNormalMatrixCol1 = [];
                this._modelNormalMatrixCol2 = [];
            }
    ...
    setColor(portionId, color) { // RGBA color is normalized as ints
        ...
        if (this.state.colorsBuf) {
            this.state.colorsBuf.setData(tempUint8Vec4, portionId * 4, 4);
        }
    }
    ...
    drawColorOpaque(renderFlags: RenderFlags, frameCtx: FrameContext): void {
        ...
        if (frameCtx.withSAO && this.model.saoEnabled) {
            if (frameCtx.pbrEnabled && this.model.pbrEnabled && this.state.pbrSupported) {
            ...
            } else if (frameCtx.colorTextureEnabled && this.model.colorTextureEnabled && this.state.colorTextureSupported) {
        ...
        } else if (frameCtx.pbrEnabled && this.model.pbrEnabled && this.state.pbrSupported) {
    ...
    #updateBackfaceCull(renderFlags: RenderFlags, frameCtx: FrameContext): void {
    ...
    drawEdgesColorOpaque(renderFlags: RenderFlags, frameCtx: FrameContext): void {
        if (this.#numCulledLayerPortions === this.#numPortions || this.#numVisibleLayerPortions === 0 || this._numEdgesLayerPortions === 0) {
            return;
        }
    ...
    precisionRayPickSurface(portionId, worldRayOrigin, worldRayDir, worldSurfacePos, worldNormal) {
        ...
        for (let i = 0, len = indices.length; i < len; i += 3) {
            ...
            math.decompressPosition(a, state.positionsDecompressMatrix);
            math.decompressPosition(b, state.positionsDecompressMatrix);
            math.decompressPosition(c, state.positionsDecompressMatrix);
