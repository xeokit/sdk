...
import {utils, Map} from './utils';

const stateSortIds = new Map({});

/**
 * @desc Base class for all core xeokit components.
 *
 * ## Component IDs
 *
...
class Component {
    ...
    _attach(params) {
        ...
        if (component) {
            ...
            if (recompiles) {
                attachment.subs.push(component.on("dirty", () => {
    ...
    _getStateSortId() {
        return stateSortIds.addItem({});
    }

    _putStateSortId(stateSortId) {
        stateSortIds.removeItem(stateSortId);
    }
