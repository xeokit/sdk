...
export class DataTextureFactory {
     ...
     * @param normalMatrices Normals instancing matrix for each mesh in the layer. Null if the meshes are not instanced.
     */
    createEachMeshMatricesDataTexture(gl: WebGL2RenderingContext, positionDecodeMatrices: math.FloatArrayType, matrices: math.FloatArrayType, normalMatrices: math.FloatArrayType): DataTexture {
        ...
        for (let i = 0; i < positionDecodeMatrices.length; i++) {
            ...
            textureData.set(normalMatrices [i], i * 48 + 32);   // 4x4 values
        }
        const texture = gl.createTexture();
        gl.bindTexture(gl.TEXTURE_2D, texture);
     ...
     * @param meshIds
     */
    createPointerTableDataTexture(gl: WebGL2RenderingContext, meshIds: math.IntArrayType): DataTexture {
        if (meshIds.length == 0) {
            return emptyDataTexture;
        }
