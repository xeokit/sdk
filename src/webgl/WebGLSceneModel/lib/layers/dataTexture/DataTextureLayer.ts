...
export class DataTextureLayer {
    ...
    constructor(params: BatchingLayerParams, renderers: any) {
        ...
        this.#buffer = new DataTextureBuffer(params.maxGeometryBatchSize);
     ...
     */
    canCreatePortion(geometryCfg: { preparedBuckets: any[]; indices: string | any[]; }, instancingGeometryId:any = null) {
        if (this.#finalized) {
     ...
     */
    createPortion(geometryCfg: {
                      aabb: any;
                      preparedBuckets: any[];
                      id: string;
                      positionsDecodeMatrix: any;
                      origin: any;
                  },
                  objectCfg:any = null) {
        ...
        geometryCfg.preparedBuckets.forEach((bucketGeometry, bucketIndex) => {
            ...
            if (instancing) {
                ...
                if (!(key in this.#instancedGeometrySubPortionData)) {
                    // @ts-ignore
                    this.#instancedGeometrySubPortionData[key] = this.createSubPortionGeometry(bucketGeometry);
                }

                geometrySubPortionData = this.#instancedGeometrySubPortionData[key];
            } else {
     ...
     * @private
     */
    createSubPortionObject(params: { color: any; metallic: any; roughness: any; colors: any; opacity: any; meshMatrix: any; worldMatrix: any; pickColor: any; }, geometrySubPortionData: { vertexBase: any; numVertices: number; indicesBase: number; edgeIndicesBase: number; numTriangles: number; numEdges: number; }, positionsCompressed: string | any[] | Uint8Array | Int8Array | Uint16Array | Uint32Array | Int16Array | Float32Array | Float64Array, positionsDecodeMatrix: math.FloatArrayType, origin: any[], worldAABB: any[] | Uint8Array | Int8Array | Uint16Array | Uint32Array | Int16Array | Float32Array | Float64Array, instancing: boolean) {
    ...
    finalize() {
        ...
        const state = this.state;
        const textureState = this.#dataTextureState;
        ...
        if (!this.sceneModel._modelMatricesTexture) {
            this.sceneModel._modelMatricesTexture = this.dataTextureGenerator.generateVBOSceneModelDataTexture(
                gl, this.sceneModel
            );
        }

        textureState.textureModelMatrices = this.sceneModel._modelMatricesTexture;

        // Camera textures
        if (!this.sceneModel.cameraTexture) {
            this.sceneModel.cameraTexture = this.dataTextureGenerator.generateCameraDataTexture(
                this.sceneModel.scene.canvas.gl,
                this.sceneModel.scene.camera,
                this.sceneModel.scene,
                this.state.origin.slice()
            );
        }

        textureState.textureCameraMatrices = this.sceneModel.cameraTexture;
    ...
    initFlags(portionId: any, flags: number, meshTransparent: any) {
    ...
    setVisible(portionId: any, flags: number, transparent: any) {
        if (!this.#finalized) {
            throw "Not finalized";
        }
        if (flags & ENTITY_FLAGS.VISIBLE) {
    ...
    #setFlags2(portionId, flags, deferred = false) {
        const subPortionMapping = this.#subPortionIdMapping[portionId];
        for (let i = 0, len = subPortionMapping.length; i < len; i++) {
    ...
    #subPortionSetFlags2(portionId, flags, deferred = false) {
        if (!this.#finalized) {
            throw "Not finalized";
        }
    ...
    setOffset(portionId: string | number, offset: number[]) {
        const subPortionMapping = this.#subPortionIdMapping[portionId];
    ...
    drawColorOpaque(drawFlags: DrawFlags, frameCtx: FrameContext) {
        ...
        this.#updateBackfaceCull(drawFlags, frameCtx);
        if (frameCtx.withSAO && this.sceneModel.qualityRender) {
            if (frameCtx.pbrEnabled && this.sceneModel.qualityRender) {
                if (this.#dataTextureRenderers.colorQualityRendererWithSAO) {
                    this.#dataTextureRenderers.colorQualityRendererWithSAO.drawLayer(frameCtx, this, RENDER_PASSES.COLOR_OPAQUE);
                }
            } else {
                if (this.#dataTextureRenderers.colorRendererWithSAO) {
                    this.#dataTextureRenderers.colorRendererWithSAO.drawLayer(frameCtx, this, RENDER_PASSES.COLOR_OPAQUE);
                }
            }
        } else {
            if (frameCtx.pbrEnabled && this.sceneModel.qualityRender) {
                if (this.#dataTextureRenderers.colorQualityRenderer) {
                    this.#dataTextureRenderers.colorQualityRenderer.drawLayer(frameCtx, this, RENDER_PASSES.COLOR_OPAQUE);
                }
            } else {
                if (this.#dataTextureRenderers.colorRenderer) {
                    this.#dataTextureRenderers.colorRenderer.drawLayer(frameCtx, this, RENDER_PASSES.COLOR_OPAQUE);
                }
            }
        }
    }

    #updateBackfaceCull(drawFlags: DrawFlags, frameCtx: FrameContext) {
    ...
    drawColorTransparent(drawFlags: DrawFlags, frameCtx: FrameContext) {
        ...
        if (frameCtx.pbrEnabled && this.sceneModel.qualityRender) {
            if (this.#dataTextureRenderers.colorQualityRenderer) {
                this.#dataTextureRenderers.colorQualityRenderer.drawLayer(frameCtx, this, RENDER_PASSES.COLOR_TRANSPARENT);
            }
        } else {
            if (this.#dataTextureRenderers.colorRenderer) {
                this.#dataTextureRenderers.colorRenderer.drawLayer(frameCtx, this, RENDER_PASSES.COLOR_TRANSPARENT);
            }
        }
    }
    ...
    drawDepth(drawFlags: DrawFlags, frameCtx: FrameContext) {
        if (this.#numCulledLayerPortions === this.#numPortions || this.#numVisibleLayerPortions === 0 || this.#numTransparentLayerPortions === this.#numPortions || this.#numXRayedLayerPortions === this.#numPortions) {
            return;
        }
        this.#updateBackfaceCull(drawFlags, frameCtx);
        if (this.#dataTextureRenderers.depthRenderer) {
            this.#dataTextureRenderers.depthRenderer.drawLayer(frameCtx, this, RENDER_PASSES.COLOR_OPAQUE); // Assume whatever post-effect uses depth (eg SAO) does not apply to transparent objects
        }
    }

    drawNormals(drawFlags: DrawFlags, frameCtx: FrameContext) {
        if (this.#numCulledLayerPortions === this.#numPortions || this.#numVisibleLayerPortions === 0 || this.#numTransparentLayerPortions === this.#numPortions || this.#numXRayedLayerPortions === this.#numPortions) {
            return;
        }
        this.#updateBackfaceCull(drawFlags, frameCtx);
        if (this.#dataTextureRenderers.normalsRenderer) {
            this.#dataTextureRenderers.normalsRenderer.drawLayer(frameCtx, this, RENDER_PASSES.COLOR_OPAQUE);  // Assume whatever post-effect uses normals (eg SAO) does not apply to transparent objects
        }
    }

    // ---------------------- SILHOUETTE RENDERING -----------------------------------

    drawSilhouetteXRayed(drawFlags: DrawFlags, frameCtx: FrameContext) {
        ...
        if (this.#dataTextureRenderers.silhouetteRenderer) {
            this.#dataTextureRenderers.silhouetteRenderer.drawLayer(frameCtx, this, RENDER_PASSES.SILHOUETTE_XRAYED);
        }
    }

    drawSilhouetteHighlighted(drawFlags: DrawFlags, frameCtx: FrameContext) {
        if (this.#numCulledLayerPortions === this.#numPortions || this.#numVisibleLayerPortions === 0 || this.#numHighlightedLayerPortions === 0) {
            return;
        }
        this.#updateBackfaceCull(drawFlags, frameCtx);
        if (this.#dataTextureRenderers.silhouetteRenderer) {
            this.#dataTextureRenderers.silhouetteRenderer.drawLayer(frameCtx, this, RENDER_PASSES.SILHOUETTE_HIGHLIGHTED);
        }
    }

    drawSilhouetteSelected(drawFlags: DrawFlags, frameCtx: FrameContext) {
        if (this.#numCulledLayerPortions === this.#numPortions || this.#numVisibleLayerPortions === 0 || this.#numSelectedLayerPortions === 0) {
            return;
        }
        this.#updateBackfaceCull(drawFlags, frameCtx);
        if (this.#dataTextureRenderers.silhouetteRenderer) {
            this.#dataTextureRenderers.silhouetteRenderer.drawLayer(frameCtx, this, RENDER_PASSES.SILHOUETTE_SELECTED);
        }
    }

    // ---------------------- EDGES RENDERING -----------------------------------

    drawEdgesColorOpaque(drawFlags: DrawFlags, frameCtx: FrameContext) {
        ...
        if (this.#dataTextureRenderers.edgesColorRenderer) {
            this.#dataTextureRenderers.edgesColorRenderer.drawLayer(frameCtx, this, RENDER_PASSES.EDGES_COLOR_OPAQUE);
        }
    }

    drawEdgesColorTransparent(drawFlags: DrawFlags, frameCtx: FrameContext) {
        ...
        if (this.#dataTextureRenderers.edgesColorRenderer) {
            this.#dataTextureRenderers.edgesColorRenderer.drawLayer(frameCtx, this, RENDER_PASSES.EDGES_COLOR_TRANSPARENT);
        }
    }

    drawEdgesHighlighted(drawFlags: DrawFlags, frameCtx: FrameContext) {
        if (this.#numCulledLayerPortions === this.#numPortions || this.#numVisibleLayerPortions === 0 || this.#numHighlightedLayerPortions === 0) {
            return;
        }
        if (this.#dataTextureRenderers.edgesRenderer) {
            this.#dataTextureRenderers.edgesRenderer.drawLayer(frameCtx, this, RENDER_PASSES.EDGES_HIGHLIGHTED);
        }
    }

    drawEdgesSelected(drawFlags: DrawFlags, frameCtx: FrameContext) {
        if (this.#numCulledLayerPortions === this.#numPortions || this.#numVisibleLayerPortions === 0 || this.#numSelectedLayerPortions === 0) {
            return;
        }
        if (this.#dataTextureRenderers.edgesRenderer) {
            this.#dataTextureRenderers.edgesRenderer.drawLayer(frameCtx, this, RENDER_PASSES.EDGES_SELECTED);
        }
    }

    drawEdgesXRayed(drawFlags: DrawFlags, frameCtx: FrameContext) {
        if (this.#numCulledLayerPortions === this.#numPortions || this.#numVisibleLayerPortions === 0 || this.#numXRayedLayerPortions === 0) {
            return;
        }
        if (this.#dataTextureRenderers.edgesRenderer) {
            this.#dataTextureRenderers.edgesRenderer.drawLayer(frameCtx, this, RENDER_PASSES.EDGES_XRAYED);
        }
    }

    // ---------------------- OCCLUSION CULL RENDERING -----------------------------------

    drawOcclusion(drawFlags: DrawFlags, frameCtx: FrameContext) {
        ...
        if (this.#dataTextureRenderers.occlusionRenderer) {
            this.#dataTextureRenderers.occlusionRenderer.drawLayer(frameCtx, this, RENDER_PASSES.COLOR_OPAQUE);
        }
    }

    // ---------------------- SHADOW BUFFER RENDERING -----------------------------------

    drawShadow(drawFlags: DrawFlags, frameCtx: FrameContext) {
        if (this.#numCulledLayerPortions === this.#numPortions || this.#numVisibleLayerPortions === 0) {
            return;
        }
        this.#updateBackfaceCull(drawFlags, frameCtx);
        if (this.#dataTextureRenderers.shadowRenderer) {
            this.#dataTextureRenderers.shadowRenderer.drawLayer(frameCtx, this, RENDER_PASSES.COLOR_OPAQUE);
        }
    }

    //---- PICKING ----------------------------------------------------------------------------------------------------

    drawPickMesh(drawFlags: DrawFlags, frameCtx: FrameContext) {
        if (this.#numCulledLayerPortions === this.#numPortions || this.#numVisibleLayerPortions === 0) {
            return;
        }
        this.#updateBackfaceCull(drawFlags, frameCtx);
        if (this.#dataTextureRenderers.pickMeshRenderer) {
            this.#dataTextureRenderers.pickMeshRenderer.drawLayer(frameCtx, this, RENDER_PASSES.PICK);
        }
    }

    drawPickDepths(drawFlags: DrawFlags, frameCtx: FrameContext) {
        if (this.#numCulledLayerPortions === this.#numPortions || this.#numVisibleLayerPortions === 0) {
            return;
        }
        this.#updateBackfaceCull(drawFlags, frameCtx);
        if (this.#dataTextureRenderers.pickDepthRenderer) {
            this.#dataTextureRenderers.pickDepthRenderer.drawLayer(frameCtx, this, RENDER_PASSES.PICK);
        }
    }

    drawPickNormals(drawFlags: any, frameCtx: FrameContext) {
        if (this.#numCulledLayerPortions === this.#numPortions || this.#numVisibleLayerPortions === 0) {
            return;
        }
        this.#updateBackfaceCull(drawFlags, frameCtx);
        if (this.#dataTextureRenderers.pickNormalsRenderer) {
            this.#dataTextureRenderers.pickNormalsRenderer.drawLayer(frameCtx, this, RENDER_PASSES.PICK);
        }
    }
    ...
    precisionRayPickSurface(portionId: string, worldRayOrigin: math.FloatArrayType, worldRayDir: any, worldSurfacePos: { set: (arg0: math.FloatArrayType) => void; }, worldNormal: math.FloatArrayType) {
        ...
        //     c[2] = positions[ic + 2];
        //
        //     math.compression.decompressPosition(a, state.positionsDecodeMatrix);
        //     math.compression.decompressPosition(b, state.positionsDecodeMatrix);
        //     math.compression.decompressPosition(c, state.positionsDecodeMatrix);
        //
