...
import {DataModelParams, Plugin, SceneModel, WebViewer} from "../../viewer/index";
import type {XKTDataSource} from "./XKTDataSource";
...
import {ParserV11} from "./parsers/ParserV11";
...
/**
 A {@link WebViewer} {@link Plugin} that loads models from xeokit's native *````.XKT````* format.
...
class XKTLoaderPlugin extends Plugin {
     ...
     */
    public includeTypes: string[] | undefined = undefined;
    ...
    /**
     ...
     Default value is ````null````.
     */
    public excludeTypes: string[] | undefined = undefined;
    ...
    private viewer: WebViewer;

    /**
     @constructor

     @param viewer - The WebViewer.
     @param cfg  - Plugin configuration.
     @param cfg.id - ID for this plugin, so that we can find it within {@link WebViewer.plugins}. default value is "XKTLoader".
     ...
     */
    constructor(viewer: WebViewer, cfg: {
        includeTypes?: string[];
        excludeTypes?: string[];
    ...
    /**
     Loads an ````.xkt```` model into this XKTLoaderPlugin's {@link WebViewer}.

     @param params - Loading parameters.
     @param params.id - ID to assign to the root {@link Entity.id}, unique among all components in the WebViewer's {@link Scene}, generated automatically by default.
     ...
     @param params.backfaces - When we set this ````true````, then we force rendering of backfaces for the model. When we leave this ````false````, then we allow the WebViewer to decide when to render backfaces. In that case, the WebViewer will hide backfaces on watertight meshes, show backfaces on open meshes, and always show backfaces on meshes when we slice them open with {@link SectionPlane}s.  Default is ````false````.
     ...
     @returns {SceneModel} - SceneModel representing the model, which will be registered by {@link SceneModel.id} in {@link Data.models}.
     */
    load(params: {
        objectDefaults?: { [key: string]: any };
        excludeUnclassifiedObjects?: boolean;
        ...
        if (params.id && this.viewer.scene.models[params.id]) {
            console.error("Model with this ID already exists in viewer: " + params.id + " - will autogenerate this ID");
            delete params.id;
        }
        ...
        if (!params.src && !params.xkt) {
            console.error("load() param expected: src or xkt");
            ...
            if (params.dataModelSrc) {
                ...
                this.dataSource.getModelData(dataModelSrc, (dataModelData: any) => {
                    ...
                    if (!processModelDataData(dataModelData)) {
                        console.error(`load(): Failed to load model metadata for model '${modelId} from '${dataModelSrc}' - metadata not valid`);
                        sceneModel.events.fire("error", "Metadata not valid");
                    }
                    //      this.viewer.scene.canvas.spinner.processes--;
                }, (errMsg: string) => {
                    console.error(`load(): Failed to load model metadata for model '${modelId} from  '${dataModelSrc}' - ${errMsg}`);
                ...
                if (!processModelDataData(params.dataModelData)) {
                    console.error(`load(): Failed to load model metadata for model '${modelId} from '${params.dataModelSrc}' - metadata not valid`);
                    sceneModel.events.fire("error", "Metadata not valid");
                }
            }
        } else {
    ...
    #parseModel(arrayBuffer: ArrayBuffer | undefined, params: {}, options: {}, sceneModel) {
        if (sceneModel.destroyed) {
            return;
        }
        ...
        if (!parser) {
            console.error("Unsupported .XKT file version: " + xktVersion + " - this XKTLoaderPlugin supports versions " + Object.keys(parsers));
            return;
        }
        console.log("Loading .xkt V" + xktVersion);
